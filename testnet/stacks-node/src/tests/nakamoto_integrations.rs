...
///  to Nakamoto operation (activating pox-4 by submitting a stack-stx tx). The BootLoop
///  struct handles the epoch-2/3 tear-down and spin-up.
/// This test makes these assertions:
///  * When a new tenure starts, the flash blocks for the old one stop
///  * The tenure height is increased by 1
///  * The block_consensus_hash is different from the previous one
///  * Mine 2 tenures
///
///  * Each tenure has 6 blocks (the coinbase block and 5 interim blocks)
///  * TODO: does this happen here? Both nodes see the same chainstate at the end of the test
fn new_tenure_stop_fast_blocks() {
    if env::var("BITCOIND_TEST") != Ok("1".into()) {
        return;
    }
    let (mut naka_conf, _miner_account) = naka_neon_integration_conf(None);
    ...
    // TODO:
    // in 1st tenure, miner_A mines the blocks normally and signers are following miner_A
    // in 2nd tenure,
    // miner_B mines the blocks,
    // the signers are following miner_A
    // miner_B tries to mine 1 more block, the signers will not sign it -> it will not be added
    // miner_A should continue to mine the blocks as he has the right view of the chain
    ...
    for tenure_ix in 0..tenure_count {
        ...
        let block_snapshot: stacks::chainstate::burn::BlockSnapshot =
            SortitionDB::get_all_snapshots_for_burn_block(sortdb.conn(), &burnchain_header_hash)
                .unwrap()[0]
                .clone();
        // let miner_hash = block_snapshot.miner_pk_hash.unwrap();
        let winning_block_txid: Txid = block_snapshot.winning_block_txid;
        info!("Winner Miner txid: {}", winning_block_txid);

        let blocks_commits = get_block_commit_by_txid(
            sortdb.conn(),
            &block_snapshot.sortition_id,
            &winning_block_txid,
        )
        .unwrap()
        .unwrap();
        ...
        // 2 apparent_sender -> bitcoind address of miner + txid
        // table snapshots:
        //   block_height -> what we want
        //  winning_block_txid -> check with the previous who won the block

        // if tenure_ix == 1 {  // Second tenure (0-indexed)
        //     assert_eq!(
        //     anchor_miner, miner_b_address,
        //         "Expected miner B to mine the second tenure's anchor block"
        //     );
        // }
