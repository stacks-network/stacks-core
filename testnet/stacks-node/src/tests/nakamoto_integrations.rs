...
fn simple_neon_integration() {
    ...
    let tip_info = get_chain_info(&naka_conf);
    info!("Bootstrapped to Epoch-3.0 boundary, starting nakamoto miner"; "stacks_block_height" => tip_info.stacks_tip_height, "burn_block_height" => tip_info.burn_block_height);
    ...
    info!("Nakamoto miner started... waiting for a block commit");
    blind_signer(&naka_conf, &signers, proposals_submitted);

    let start = Instant::now();
    while commits_submitted.load(Ordering::SeqCst) < 1 {
