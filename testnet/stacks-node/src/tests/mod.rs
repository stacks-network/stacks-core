...
fn test_btc_to_sat() {
    ...
    for (input, expected_output) in inputs.iter().zip(expected_outputs.iter()) {
        let output = UTXO::serialized_btc_to_sat(input).unwrap(); 
...
fn test_btc_to_sat_errors() {
    assert!(UTXO::serialized_btc_to_sat("0.000000001").is_none());
    assert!(UTXO::serialized_btc_to_sat("1").is_none());
    assert!(UTXO::serialized_btc_to_sat("1e-8").is_none());
    assert!(UTXO::serialized_btc_to_sat("7.4e-7").is_none());
    assert!(UTXO::serialized_btc_to_sat("5.96e-6").is_none());
}
