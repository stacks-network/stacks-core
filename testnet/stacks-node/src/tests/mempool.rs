...
fn mempool_setup_chainstate() {
    ...
    run_loop.callbacks.on_new_tenure(|round, _burnchain_tip, chain_tip, tenure| {
        ...
        if round == 1 {
            ...
            let publish_tx4 = make_contract_publish(&contract_sk, 3, 100, "implement-trait-contract", IMPLEMENT_TRAIT_CONTRACT);
            tenure.mem_pool.submit_raw(&consensus_hash, &header_hash, publish_tx4).unwrap();

            let publish_tx4 = make_contract_publish(&contract_sk, 4, 100, "bad-trait-contract", BAD_TRAIT_CONTRACT);
            tenure.mem_pool.submit_raw(&consensus_hash, &header_hash, publish_tx4).unwrap();
        }
    });
    ...
    run_loop.callbacks.on_new_stacks_chain_state(|round, _burnchain_tip, chain_tip, chain_state, _burn_dbconn| {
        ...
        if round == 3 {
            ...
            let tx_bytes = make_contract_publish(&contract_sk, 5, 1000, "bar_contract", FOO_CONTRACT);
            ...
            let tx_bytes = make_contract_call(&contract_sk, 5, 200, &contract_addr, "foo_contract", "bar", &[Value::UInt(1)]);
            let tx = StacksTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
            chain_state.will_admit_mempool_tx(mempool_conn, consensus_hash, block_hash, &tx, tx_bytes.len() as u64).unwrap();
            ...
            let tx_bytes = make_contract_call(&contract_sk, 5, 200, &bad_addr, "foo_contract", "bar", &[Value::UInt(1), Value::Int(2)]);
            let tx = StacksTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
            let e = chain_state.will_admit_mempool_tx(mempool_conn, consensus_hash, block_hash, &tx, tx_bytes.len() as u64).unwrap_err();
            ...
            let e = chain_state.will_admit_mempool_tx(mempool_conn, consensus_hash, block_hash, &tx, tx_bytes.len() as u64).unwrap_err();
            eprintln!("Err: {:?}", e);
            assert!(if let MemPoolRejection::NotEnoughFunds(111000, 99500) = e { true } else { false });

            let tx_bytes = make_stacks_transfer(&contract_sk, 5, 99700, &other_addr, 1000);
            let tx = StacksTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
            let e = chain_state.will_admit_mempool_tx(mempool_conn, consensus_hash, block_hash, &tx, tx_bytes.len() as u64).unwrap_err();
            eprintln!("Err: {:?}", e);
            assert!(if let MemPoolRejection::NotEnoughFunds(100700, 99500) = e { true } else { false });

            let tx_bytes = make_contract_call(&contract_sk, 5, 200, &contract_addr, "bar_contract", "bar", &[Value::UInt(1)]);
            let tx = StacksTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
            let e = chain_state.will_admit_mempool_tx(mempool_conn, consensus_hash, block_hash, &tx, tx_bytes.len() as u64).unwrap_err();
            eprintln!("Err: {:?}", e);
            assert!(if let MemPoolRejection::NoSuchContract = e { true } else { false });

            let tx_bytes = make_contract_call(&contract_sk, 5, 200, &contract_addr, "foo_contract", "foobar", &[Value::UInt(1)]);
            let tx = StacksTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
            let e = chain_state.will_admit_mempool_tx(mempool_conn, consensus_hash, block_hash, &tx, tx_bytes.len() as u64).unwrap_err();
            eprintln!("Err: {:?}", e);
            assert!(if let MemPoolRejection::NoSuchPublicFunction = e { true } else { false });

            let tx_bytes = make_contract_call(&contract_sk, 5, 200, &contract_addr, "foo_contract", "bar", &[Value::UInt(1), Value::Int(2)]);
            let tx = StacksTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
            let e = chain_state.will_admit_mempool_tx(mempool_conn, consensus_hash, block_hash, &tx, tx_bytes.len() as u64).unwrap_err(); 
            eprintln!("Err: {:?}", e);
            assert!(if let MemPoolRejection::BadFunctionArgument(_) = e { true } else { false });

            let tx_bytes = make_contract_publish(&contract_sk, 5, 1000, "foo_contract", FOO_CONTRACT);
            let tx = StacksTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
            let e = chain_state.will_admit_mempool_tx(mempool_conn, consensus_hash, block_hash, &tx, tx_bytes.len() as u64).unwrap_err();
            eprintln!("Err: {:?}", e);
            ...
            let contract_id = QualifiedContractIdentifier::new(StandardPrincipalData::from(contract_addr.clone()), ContractName::try_from("implement-trait-contract").unwrap());
            let contract_principal = PrincipalData::Contract(contract_id.clone());

            let tx_bytes = make_contract_call(&contract_sk, 5, 250, &contract_addr, "use-trait-contract", "baz", &[Value::Principal(contract_principal)]);
            let tx = StacksTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
            chain_state.will_admit_mempool_tx(mempool_conn, consensus_hash, block_hash, &tx, tx_bytes.len() as u64).unwrap();

            let contract_id = QualifiedContractIdentifier::new(StandardPrincipalData::from(contract_addr.clone()), ContractName::try_from("bad-trait-contract").unwrap());
            let contract_principal = PrincipalData::Contract(contract_id.clone());
