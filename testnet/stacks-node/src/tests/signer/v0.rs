...
fn allow_reorg_within_first_proposal_burn_block_timing_secs() {
    ...
    let block_n2_hash = last_block.block_hash.clone();
    ...
    .expect("Timed out waiting for Miner 1 to Mine Block N+3");

    let last_block = get_last_observed_block();
    assert_eq!(last_block.block_height, block_n_height + 3);
    assert_eq!(last_block.parent_block_hash, block_n2_hash);
...
fn disallow_reorg_within_first_proposal_burn_block_timing_secs_but_more_than_one_block() {
    ...
    next_block_and(
        &mut signer_test.running_nodes.btc_regtest_controller,
        30,
        || Ok(signer_test.running_nodes.nakamoto_blocks_rejected.get() > 0),
    )
    .expect("Timed out waiting for Miner 1 to Mine Block N+1' and being rejected");
