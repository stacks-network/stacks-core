        ...
        match StacksChainState::get_anchored_block_header_info(chainstate.db(), &ch, &bhh) {
            Ok(Some(_)) => {
                ...
                microblock_miner_state.replace(MicroblockMinerState {
                    ...
                    frequency: config_handle.get().node.microblock_frequency,
                    last_mined: 0,
                    quantity: 0,
...
fn spawn_peer(
    ...
    let config_handle = runloop.config_handle().clone();
        ...
        .spawn(move || {
            ...
            while should_keep_running.load(Ordering::SeqCst) {
                ...
                let poll_ms = if !download_backpressure && this.has_more_downloads() {
                    ...
                    cmp::min(poll_timeout, config_handle.get().node.microblock_frequency)
                };
                    ...
                    Ok(network_result) => {
                        ...
                        if mblock_deadline < get_epoch_time_ms() {
                            ...
                            mblock_deadline = get_epoch_time_ms()
                                + (config_handle.get().node.microblock_frequency as u128);
                        }
                    }
                    Err(e) => {
...
fn spawn_miner_relayer(
    ...
    let config = runloop.config().clone();
    let config_loader_handle = runloop.config_loader_handle().clone();
    let config_handle = runloop.config_handle().clone();
    let event_dispatcher = runloop.get_event_dispatcher();
    ...
    let mut bitcoin_controller =
        BitcoinRegtestController::new_dummy(config.clone(), config_handle.clone());
            ...
            match directive {
                ...
                RelayerDirective::RunTenure(registered_key, last_burn_block, issue_timestamp_ms) => {
                    ...
                    if burn_chain_tip == burn_header_hash {
                        ...
                        if issue_timestamp_ms < last_tenure_issue_time + (config_handle.get().node.wait_time_for_microblocks as u128) {
                    ...
                    config_loader_handle.get().reload();

                    let burn_fee_cap = config_handle.get().burnchain.burn_fee_cap;
