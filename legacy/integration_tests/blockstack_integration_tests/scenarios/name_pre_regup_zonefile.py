...
def scenario( wallets, **kw ):
    ...
    testlib.blockstack_name_register( "foo1.test", wallets[2].privkey, wallets[3].addr, zonefile_hash=blockstack.lib.storage.get_zonefile_data_hash(zonefiles['foo1.test']) )
    testlib.blockstack_name_register( "foo2.test", wallets[2].privkey, wallets[3].addr, zonefile_hash=blockstack.lib.storage.get_zonefile_data_hash(zonefiles['foo2.test']) )
    testlib.blockstack_name_register( "foo3.test", wallets[2].privkey, wallets[3].addr, zonefile_hash=blockstack.lib.storage.get_zonefile_data_hash(zonefiles['foo3.test']) )
    testlib.next_block( **kw )
    ...
    for i in xrange(1, 4):
        ...
        if not res.has_key('zonefile_hash') or res['zonefile_hash'] != blockstack.lib.storage.get_zonefile_data_hash(zonefiles[name]):
            print res
            return False
