...
def scenario( wallets, **kw ):
    ...
    for i in xrange(0, 10):
        ...
        res = testlib.blockstack_name_register( "foo_{}.test".format(i), wallets[2].privkey, wallets[3].addr, zonefile_hash=value_hash)
        if 'error' in res:
            print json.dumps(res)
            return False
    ...
    for i in xrange(0, 10):
        empty_zonefile_str = testlib.make_empty_zonefile( "foo_{}.test".format(i), wallets[3].addr)
        testlib.blockstack_put_zonefile(empty_zonefile_str)

    
    # ports
    atlas_nodes = [17000, 17001, 17002, 17003, 17004, 17005, 17006, 17007]

    # try to do a peer exchange for non-existant nodes.
    # they should NOT be added to the peer set, since they can't be reached
    for port in atlas_nodes:
        res = blockstack.lib.client.atlas_peer_exchange('http://localhost:16264', 'http://localhost:{}'.format(port))
        if 'error' in res:
            print res
            return False
        ...
        # no peer should be in the list of peers (yet), since we can't reach any of them 
        for p in atlas_nodes:
            if 'localhost:{}'.format(p) in res['peers']:
                print 'have peer {}'.format(p)
                return False

        time.sleep(1)

    # get_atlas_peers() should ahve the same above behavior 
    res = blockstack.lib.client.get_atlas_peers('http://localhost:16264')
    print res

    for port in atlas_nodes:
        if 'localhost:{}'.format(port) in res['peers']:
            print 'have peer {}'.format(p)
            return False
    ...
    # make sure our peer set is full 
    res = blockstack.lib.client.get_atlas_peers('http://localhost:16264')
    print res

    absent = []
    for port in atlas_nodes:
        if 'localhost:{}'.format(port) not in res['peers']:
            absent.append(port)
