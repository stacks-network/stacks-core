...
def scenario( wallets, **kw ):
    ...
    # transfer 
    resp = testlib.blockstack_name_transfer('foo.test', wallets[1].addr, True, wallets[4].privkey)
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        return False
    ...
    # transfer 
    resp = testlib.blockstack_name_transfer('foo.test', wallets[2].addr, True, wallets[0].privkey)
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        return False
    ...
    # renew/transfer 
    resp = testlib.blockstack_name_renew( "foo.test", wallets[2].privkey, recipient_addr=wallets[1].addr )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        return False
