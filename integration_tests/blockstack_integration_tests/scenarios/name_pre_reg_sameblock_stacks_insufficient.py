...
wallets = [
    ...
    testlib.Wallet( "5Kg5kJbQHvk1B64rJniEmgbD83FpZpbw2RjdAZEzTefs9ihN3Bz", 3 * 110912000 - 1), # enough for 2 names, but not quite 3
...
def check( state_engine ):
    ...
    for name in ['baz.test', 'goo.test']:
        if state_engine.get_name(name) is not None:
            print 'accidentally registered {}'.format(name)
            return False

    # goo.test preorder should exist
    if state_engine.get_name_preorder('goo.test', virtualchain.make_payment_script(wallets[2].addr), wallets[3].addr ) is None:
        print 'no preorder for goo.test'
        return False

    # baz.test preorder should not exist
    if state_engine.get_name_preorder('baz.test', virtualchain.make_payment_script(wallets[2].addr), wallets[3].addr ):
        print 'no preorder for baz.test'
        return False
