...
def scenario( wallets, **kw ):
    ...
    for i in xrange(0, len(names)):
        ...
        resp = testlib.blockstack_name_preorder( name, preorder_wallet.privkey, register_wallet.addr )
        if debug or  'error' in resp:
            print json.dumps( resp, indent=4 )
    ...
    for i in xrange(0, len(names)):
        ...
        resp = testlib.blockstack_name_register( name, preorder_wallet.privkey, register_wallet.addr )
        if debug or  'error' in resp:
            print json.dumps( resp, indent=4 )
