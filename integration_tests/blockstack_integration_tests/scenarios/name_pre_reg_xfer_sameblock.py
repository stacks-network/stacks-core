...
def scenario( wallets, **kw ):
    ...
    # should fail
    testlib.blockstack_name_transfer('foo.test', wallets[4].addr, True, wallets[3].privkey, safety_checks=False, tx_fee=30000)

    testlib.next_block( **kw )
    transfer_block = testlib.get_current_block(**kw)
    testlib.expect_snv_fail_at('foo.test', transfer_block)
