...
def scenario( wallets, **kw ):
    ...
    res = testlib.blockstack_cli_app_publish("foo.test", "names,register,prices,user_read,user_write,user_admin,zonefiles,revoke", index_file_path, appname="register", drivers="disk", password="0123456789abcdef" )
    if 'error' in res:
    ...
    # present?
    res = testlib.blockstack_REST_call('GET', '/v1/names', ses, name='foo.test', appname='register')
    ...
    # revoke it
    res = testlib.blockstack_REST_call("DELETE", "/v1/names/bar.test", ses, name="foo.test", appname="register")
    if 'error' in res or res['http_status'] != 202:
