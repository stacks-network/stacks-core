...
def scenario( wallets, **kw ):
    ...
    for name in names.keys():
        ...
        resp = testlib.blockstack_name_preorder( name, names[name][1].privkey, names[name][0].addr )
        if debug or  'error' in resp:
            print json.dumps( resp, indent=4 )
    ...
    for name in names.keys():
        ...
        resp = testlib.blockstack_name_register( name, names[name][1].privkey, names[name][0].addr )
        if debug or  'error' in resp:
            print json.dumps( resp, indent=4 )
