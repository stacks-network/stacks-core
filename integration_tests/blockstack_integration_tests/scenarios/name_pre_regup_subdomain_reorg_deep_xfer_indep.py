...
def scenario( wallets, **kw ):
    ...
    # all zone files will be updated via a different name (foo4.test)
    def _send_batch(zf_default_url, seq, new_addr, cur_privkey):
        zf_template = "$ORIGIN {}\n$TTL 3600\n{}"

        zonefiles = {
            'foo1.test': zf_template.format('foo4.test', subdomains.make_subdomain_txt('bar.foo1.test', 'foo4.test', new_addr, seq, zf_template.format('bar.foo1.test', zf_default_url), cur_privkey)),
            'foo2.test': zf_template.format('foo4.test', subdomains.make_subdomain_txt('bar.foo2.test', 'foo4.test', new_addr, seq, zf_template.format('bar.foo2.test', zf_default_url), cur_privkey)),
            'foo3.test': zf_template.format('foo4.test', subdomains.make_subdomain_txt('bar.foo3.test', 'foo4.test', new_addr, seq, zf_template.format('bar.foo3.test', zf_default_url), cur_privkey)),
        }
