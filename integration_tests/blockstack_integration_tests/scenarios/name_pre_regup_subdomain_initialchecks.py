...
def scenario( wallets, **kw ):
    ...
    # all subdomains should be rejected, since a sequence-0 subdomain can't be processed if the stem name is missing any zonefiles
    proxy = testlib.make_proxy()
    for i in xrange(1, 4):
        fqn = 'bar.foo{}.test'.format(i)
        res = client.get_name_record(fqn, proxy=proxy)
        if 'error' not in res:
            print 'succeeded to query {}'.format(fqn)
            return False
