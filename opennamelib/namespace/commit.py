...
def commit_renewal(db, nameop, current_block_number):
    ...
    block_last_renewed = db.name_records[name]['block_last_renewed']
    ...
    db.name_records[name]['block_last_renewed'] = current_block_number

def commit_update(db, nameop):
    db.name_records[nameop['name']]['value_hash'] = nameop['update']

def commit_transfer(db, nameop):
    db.name_records[nameop['name']]['owner'] = nameop['recipient']