...
#[test]
fn test_drop_and_blacklist_txs() {
    let mut chainstate = instantiate_chainstate(false, 0x80000000, "test_drop_and_blacklist_txs");
    let chainstate_path = chainstate_path("test_drop_and_blacklist_txs");
    let mut mempool = MemPoolDB::open_test(false, 0x80000000, &chainstate_path).unwrap();
    ...
    let mempool_tx = mempool.tx_begin().unwrap();
    MemPoolDB::garbage_collect_tx_blacklist(&mempool_tx, get_epoch_time_secs() + 1, 0).unwrap();
    mempool_tx.commit().unwrap();
    ...
    let mempool_tx = mempool.tx_begin().unwrap();
    MemPoolDB::garbage_collect_tx_blacklist(&mempool_tx, get_epoch_time_secs() + 2, 0).unwrap();
    mempool_tx.commit().unwrap();
