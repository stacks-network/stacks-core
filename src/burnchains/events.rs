...
pub struct NewBlockTxEvent {
    ...
    #[serde(default)]
    contract_event: Option<ContractEvent>,
}
...
pub struct NewBlock {
    ...
    events: Vec<NewBlockTxEvent>,
}


fn deser_clarity_value<'de, D>(deser: D) -> Result<ClarityValue, D::Error> where D: Deserializer<'de> {
    let str_val = String::deserialize(deser)?;
    ClarityValue::try_deserialize_hex_untyped(&str_val)
        .map_err(DeserError::custom)
}

fn deser_contract_identifier<'de, D>(deser: D) -> Result<QualifiedContractIdentifier, D::Error> where D: Deserializer<'de> {
    let str_val = String::deserialize(deser)?;
    QualifiedContractIdentifier::parse(&str_val)
        .map_err(DeserError::custom)
}


fn deser_txid<'de, D>(deser: D) -> Result<Txid, D::Error> where D: Deserializer<'de> {
    let str_val = String::deserialize(deser)?;
    match str_val.get(2..) {
        Some(hex) => Txid::from_hex(hex)
            .map_err(DeserError::custom),
        None => Err(DeserError::custom(HexError::BadLength(2))),
    }
}

fn deser_stacks_block_id<'de, D>(deser: D) -> Result<StacksBlockId, D::Error> where D: Deserializer<'de> {
    let str_val = String::deserialize(deser)?;
    match str_val.get(2..) {
        Some(hex) => StacksBlockId::from_hex(hex)
            .map_err(DeserError::custom),
        None => Err(DeserError::custom(HexError::BadLength(2))),
    }
}

fn deser_tx_event_type<'de, D>(deser: D) -> Result<TxEventType, D::Error> where D: Deserializer<'de> {
    let str_val = String::deserialize(deser)?;
...
impl SubnetStacksEvent {
    pub fn try_from_clar_value(v: ClarityValue,
                               txid: Txid,
                               in_block: &StacksBlockId) -> Result<SubnetStacksEvent, String> {
        ...
        let event = tuple.get("event").map_err(|_| "No 'event' field in Clarity tuple")?;
        ...
        match event.as_str() {
            "block-commit" => {
                ...
                let block_commit = if let ClarityValue::Sequence(SequenceData::Buffer(buff_data)) = block_commit {
                    if u32::from(buff_data.len()) != 32 {
                        Err("Expected 'block-commit' type to be buffer")                        
                    } else {
                        let mut buff = [0; 32];
...
impl StacksEventBlock {
    fn from_new_block_event(subnets_contract: &QualifiedContractIdentifier, b: NewBlock) -> Self {
        let NewBlock { events, index_block_hash, parent_index_block_hash, block_height, .. } = b;
