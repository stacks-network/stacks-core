...
use chainstate::keys as chain_keys;
    ...
    pub fn to_address_bits(&self) -> Vec<u8> {
        match self.in_type {
            BurnchainInputType::BitcoinInput => {
                if self.keys.len() == 1 {
                    chain_keys::to_addrbits_p2pkh(&self.keys[0])
                }
                else {
                    chain_keys::to_addrbits_p2sh(self.num_required, &self.keys)
                }
            },
            BurnchainInputType::BitcoinSegwitP2SHInput => {
                if self.keys.len() == 1 {
                    chain_keys::to_addrbits_p2sh_p2wpkh(&self.keys[0])
                }
                else {
                    chain_keys::to_addrbits_p2sh_p2wsh(self.num_required, &self.keys)
                }
            }
        }
    }
