...
impl BurnchainTxInput<BitcoinPublicKey> {
    ...
    /// a burnchain tx input
    pub fn from_bitcoin_pubkey_pushbytes(num_sigs: usize, pubkey_pushbytes: &[Instruction]) -> Option<BurnchainTxInput<BitcoinPublicKey>> {
        ...
        // public keys.
        let mut tx_input = BurnchainTxInput::<BitcoinPublicKey> {
            keys: Vec::with_capacity(pubkey_pushbytes.len()),
        ...
        for i in 0..pubkey_pushbytes.len() {
            ...
            tx_input.keys.push(pubk.unwrap());
        }
    ...
    /// extract a burnchain tx input 
    pub fn from_bitcoin_pubkey_vecs(num_sigs: usize, pubkey_vecs: &[Vec<u8>]) -> Option<BurnchainTxInput<BitcoinPublicKey>> {
        ...
        let mut tx_input = BurnchainTxInput::<BitcoinPublicKey> {
            keys: Vec::with_capacity(pubkey_vecs.len()),
            num_required: num_sigs
        };
    ...
    pub fn from_bitcoin_multisig_redeem_script(multisig_script: &Instruction) -> Option<BurnchainTxInput<BitcoinPublicKey>> {
                    ...
                    (Instruction::Op(op1), Instruction::Op(op2), Instruction::Op(btc_opcodes::OP_CHECKMULTISIG)) => {
                        ...
                        match (btc_opcodes::from(*op1).classify(), btc_opcodes::from(*op2).classify()) {
                            (Class::PushNum(num_sigs), Class::PushNum(num_pubkeys)) => {
                                ...
                                BurnchainTxInput::from_bitcoin_pubkey_pushbytes(num_sigs as usize, pubkey_pushbytes)
                            },
                            (_, _) => {
    ...
    pub fn from_bitcoin_p2sh_multisig_script_sig(instructions: &Vec<Instruction>) -> Option<BurnchainTxInput<BitcoinPublicKey>> {
        ...
        let tx_input_opt = BurnchainTxInput::from_bitcoin_multisig_redeem_script(redeem_script);
    ...
    pub fn from_bitcoin_p2wpkh_p2sh_script_sig(instructions: &Vec<Instruction>, witness: &Vec<Vec<u8>>) -> Option<BurnchainTxInput<BitcoinPublicKey>> {
        ...
        match &instructions[0] {
            Instruction::PushBytes(witness_hash) => {
                ...
                BurnchainTxInput::from_bitcoin_pubkey_vecs(1, &witness[1..])
            },
            _ => {
    ...
    pub fn from_bitcoin_p2wsh_p2sh_multisig_script_sig(instructions: &Vec<Instruction>, witness: &Vec<Vec<u8>>) -> Option<BurnchainTxInput<BitcoinPublicKey>> {
        ...
        match &instructions[0] {
            Instruction::PushBytes(witness_hash) => {
                ...
                let tx_input_opt = BurnchainTxInput::from_bitcoin_multisig_redeem_script(&Instruction::PushBytes(&redeem_script[..]));
                if tx_input_opt.is_none() {
                    return None;
                }
...
impl BurnchainTxOutput {

    /// Parse a BurnchainTxOutput from a Bitcoin scriptpubkey and its value in satoshis
    pub fn from_bitcoin_script_pubkey(script_pubkey: &Script, amount: u64) -> Option<BurnchainTxOutput> {
        let script_bytes = script_pubkey.to_bytes();
        let (address_type, address_bytes) =
            if script_pubkey.is_p2pkh() {
                // TODO: is there a more idiomatic way to do this?
                let mut hash160_bytes = [0; 20];
                for i in 0..20 {
                    hash160_bytes[i] = script_bytes[i+3];
                }
                (Some(AddressType::PublicKeyHash), Some(Hash160(hash160_bytes)))
            }
            else if script_pubkey.is_p2sh() {
                // TODO: is there a more idiomatic way to do this?
                let mut hash160_bytes = [0; 20];
                for i in 0..20 {
                    hash160_bytes[i] = script_bytes[i+2];
                }
                (Some(AddressType::ScriptHash), Some(Hash160(hash160_bytes)))
            }
            else {
                (None, None)
            };

        match (address_type, address_bytes) {
            (Some(addrtype), Some(bytes)) => {
    ...
    pub fn from_bitcoin_txout(txout: &BtcTxOut) -> Option<BurnchainTxOutput> {
...
mod tests {
    ...
    fn tx_output_strange() {
        ...
        let tx_fixtures_strange : Vec<ScriptFixture<Option<BurnchainTxOutput>>> = vec![
            ScriptFixture {
