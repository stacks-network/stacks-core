...
pub struct TrieCursor {
    ...
    pub diverged: bool              // flag as to whether or not the path we're walking differs from a node's compressed path
}
...
impl TrieCursor {
    ...
    /// Did we reach a node where we could not walk any further?
    ...
    pub fn retarget(&mut self, node: &TrieNodeType, ptr: &TriePtr, hash: &BlockHeaderHash) -> () {
        self.nodes.pop();
        self.node_ptrs.pop();
    ...
    /// Otherwise return None, and possibly set the 'diverged' flag (if applicable).
    ///
    /// This method does not follow back-pointers, and will return None if a back-pointer is
    ...
    pub fn walk(&mut self, node: &TrieNodeType, block_hash: &BlockHeaderHash) -> Option<TriePtr> {
        ...
        if self.index < self.path.len() {
            ...
            let do_walk = match ptr_opt {
                Some(ref ptr) => {
                    if !is_backptr(ptr.id()) {
                        ...
                        // walk_backptr_step_backptr() for each node visited, and then walk_backptr_finish()
            ...
            if ptr_opt.is_none() {
                trace!("cursor: not found: chr = 0x{:02x}, self.index = {}, self.path = {:?}", chr, self.index-1, &path_bytes);
            }
            ptr_opt
        }
        else {
    ...
    pub fn walk_backptr_step_backptr(&mut self, next_node: &TrieNodeType, ptr: &TriePtr, block_hash: &BlockHeaderHash) -> () {
        ...
        trace!("Cursor: walk_backptr_step_backptr ptr={:?} block_hash={:?} next_node={:?}", ptr, block_hash, next_node);
    }
    ...
    pub fn walk_backptr_finish(&mut self, ptr: &TriePtr, block_hash: &BlockHeaderHash) -> () {
        ...
        trace!("Cursor: walk_backptr_finish ptr={:?} block_hash={:?}", ptr, block_hash);
    }
}
