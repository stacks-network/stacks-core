...
use rusqlite::{
    blob::Blob,
    types::{FromSql, ToSql},
    Connection, Error as SqliteError, OptionalExtension, Transaction, params,
};
...
fn get_schema_version(conn: &Connection) -> u64 {
    ...
    match conn.query_row(sql, params![], |row| row.get::<_, i64>("version")) {
        Ok(x) => x as u64,
        Err(e) => {
...
pub fn get_external_blobs_length(conn: &Connection) -> Result<u64, Error> {
    ...
    let max_len = query_row(conn, qry, params![])?.unwrap_or(0);
    Ok(max_len)
}
...
pub fn clear_lock_data(conn: &Connection) -> Result<(), Error> {
    conn.execute("DELETE FROM block_extension_locks", params![])?;
    Ok(())
}

pub fn clear_tables(tx: &Transaction) -> Result<(), Error> {
    tx.execute("DELETE FROM block_extension_locks", params![])?;
    tx.execute("DELETE FROM marf_data", params![])?;
    tx.execute("DELETE FROM mined_blocks", params![])?;
    Ok(())
}
