...
pub mod test {
    ...
    fn process_token_transfer_stx_transaction() {
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            let (fee, _) =
                StacksChainState::process_transaction(&mut conn, &signed_tx, false, ASTRules::PrecheckSize).unwrap();
    ...
    fn process_token_transfer_stx_transaction_invalid() {
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            .zip(error_frags.clone())
            {
                ...
                let res = StacksChainState::process_transaction(&mut conn, &signed_tx, false, ASTRules::PrecheckSize);
                assert!(res.is_err());
    ...
    fn process_smart_contract_contract_call_transaction() {
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            let (fee_2, _) =
                StacksChainState::process_transaction(&mut conn, &signed_tx_2, false, ASTRules::PrecheckSize).unwrap();
    ...
    fn process_smart_contract_contract_call_invalid() {
        ...
        for (dbi, burn_db) in PRE_21_DBS.iter().enumerate() {
            ...
            for contract_call in contract_calls.iter() {
                ...
                let res = StacksChainState::process_transaction(&mut conn, &signed_tx_2, false, ASTRules::PrecheckSize);
                assert!(res.is_err());
    ...
    fn process_post_conditions_tokens() {
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            // publish contract
            let _ = StacksChainState::process_transaction(&mut conn, &signed_contract_tx, false, ASTRules::PrecheckSize)
                .unwrap();
            ...
            for tx_pass in post_conditions_pass.iter() {
                let (_fee, _) =
                    StacksChainState::process_transaction(&mut conn, &tx_pass, false, ASTRules::PrecheckSize).unwrap();
            ...
            for tx_fail in post_conditions_fail.iter() {
                let (_fee, _) =
                    StacksChainState::process_transaction(&mut conn, &tx_fail, false, ASTRules::PrecheckSize).unwrap();
                expected_nonce += 1;
...
"
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            let (_fee, receipt) =
                StacksChainState::process_transaction(&mut conn, &contract_call_tx, false, ASTRules::PrecheckSize).unwrap();
    ...
    fn process_smart_contract_fee_check() {
        ...
        for (dbi, burn_db) in PRE_21_DBS.iter().enumerate() {
            ...
            let (fee, _) =
                StacksChainState::process_transaction(&mut conn, &signed_contract_tx, false, ASTRules::PrecheckSize)
                    .unwrap();
            let err =
                StacksChainState::process_transaction(&mut conn, &signed_contract_call_tx, false, ASTRules::PrecheckSize)
                    .unwrap_err();
        ...
        let (fee, _) =
            StacksChainState::process_transaction(&mut conn, &signed_contract_tx, false, ASTRules::PrecheckSize).unwrap();
        let (fee, _) =
            StacksChainState::process_transaction(&mut conn, &signed_contract_call_tx, false, ASTRules::PrecheckSize)
                .unwrap();
    ...
    fn test_get_tx_clarity_version_v205() {
        ...
        if let Err(Error::InvalidStacksTransaction(msg, ..)) =
            StacksChainState::process_transaction(&mut conn, &smart_contract_v2, false, ASTRules::PrecheckSize)
        {
    ...
    fn process_fee_gating() {
        ...
        let err = StacksChainState::process_transaction(&mut conn, &signed_contract_call_tx, false, ASTRules::PrecheckSize)
            .unwrap_err();
        conn.commit_block();
    ...
    fn test_checkerrors_at_runtime() {
        ...
        let (fee, _) =
            StacksChainState::process_transaction(&mut conn, &signed_runtime_checkerror_tx, false, ASTRules::PrecheckSize)
                .unwrap();
        assert_eq!(fee, 1);
