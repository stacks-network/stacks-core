...
pub mod test {
    ...
    fn get_stacker_info(peer: &mut TestPeer, addr: &PrincipalData) -> Option<(u128, (AddressHashMode, Hash160), u128, u128)> {
        ...
        for tenure_id in 0..num_blocks {
            ...
            if tenure_id <= 1 {
                ...
                if cur_reward_cycle >= alice_reward_cycle {
                    ...
                    let (amount_ustx, pox_addr, lock_period, first_reward_cycle) = get_stacker_info(&mut peer, &key_to_stacks_addr(&alice).into()).unwrap();
            ...
            if tenure_id <= 1 {
                ...
                if cur_reward_cycle >= alice_reward_cycle {
                    ...
                    if cur_reward_cycle == alice_reward_cycle {
                        ...
                        let (amount_ustx, pox_addr, lock_period, first_reward_cycle) = get_stacker_info(&mut peer, &make_contract_id(&key_to_stacks_addr(&bob), "do-lockup").into()).unwrap();
        ...
        for tenure_id in 0..num_blocks {
            ...
            if first_reward_cycle > 0 && second_reward_cycle == 0 {
                if cur_reward_cycle == first_reward_cycle {
                    ...
                    let (amount_ustx, pox_addr, lock_period, first_pox_reward_cycle) = get_stacker_info(&mut peer, &key_to_stacks_addr(&alice).into()).unwrap();
                    eprintln!("\nAlice: {} uSTX stacked for {} cycle(s); addr is {:?}; first reward cycle is {}\n", amount_ustx, lock_period, &pox_addr, first_reward_cycle);
                ...
                if cur_reward_cycle == second_reward_cycle {
                    ...
                    let (amount_ustx, pox_addr, lock_period, first_pox_reward_cycle) = get_stacker_info(&mut peer, &make_contract_id(&key_to_stacks_addr(&bob), "do-lockup").into()).unwrap();
            ...
            if reward_cycle > 0 {
                if cur_reward_cycle == reward_cycle {
                    ...
                    for addr in stacker_addrs.iter() {
                        let (amount_ustx, pox_addr, lock_period, pox_reward_cycle) = get_stacker_info(&mut peer, addr).unwrap();
