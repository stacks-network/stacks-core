                            ...
                            if contract_call.to_clarity_contract_id() == boot_code_id("bns") {
                                for event in receipt.events.iter() {
                                    if let StacksTransactionEvent::SmartContractEvent(ref event_data) = event {
                                        ...
                                        // todo(ludo): these values are generated by users. it should not crash
                                        ...
                                        let page_index = attachment_data
                                            ...
                                            .expect_u128();
                                        let content_hash = {
                                            let value = attachment_data
                                                .get("hash")
                                                .expect(&format!("FATAL: no 'hash'"))
                                                .to_owned()
                                                .expect_buff(20);
                                            Hash160::from_bytes(&value[..]).unwrap()
                                        };
                                        ...
                                        let metadata = {
                                            let value = attachment_data
                                                .get("metadata")
                                                .expect(&format!("FATAL: no 'metadata'"))
                                                .to_owned();
                                            let mut serialized = vec![];
                                            value.consensus_serialize(&mut serialized)
                                        ...
                                        let request = AttachmentInstance {
                                            ...
                                            burn_block_height: 0, // todo(ludo): is this even required? block_receipt.header.anchored_header.block_height,
