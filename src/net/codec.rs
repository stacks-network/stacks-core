    ...
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        match *self {
            MemPoolSyncData::BloomFilter(ref bloom_filter) => {
                write_next(fd, &(MemPoolSyncDataID::BloomFilter as u8))?;
                write_next(fd, bloom_filter)?;
            }
            MemPoolSyncData::TxTags(ref seed, ref tags) => {
                write_next(fd, &(MemPoolSyncDataID::TxTags as u8))?;
                write_next(fd, seed)?;
                write_next(fd, tags)?;
            }
        }
