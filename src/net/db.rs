...
impl LocalPeer {
    pub fn new(network_id: u32, parent_network_id: u32, addrbytes: PeerAddress, port: u16, key_expire: u64, data_url: UrlString) -> LocalPeer {
...
impl PeerDB {
    fn instantiate(&mut self, network_id: u32, parent_network_id: u32, key_expires: u64, data_url: UrlString, p2p_addr: PeerAddress, p2p_port: u16, asn4_entries: &Vec<ASEntry4>, initial_neighbors: &Vec<Neighbor>) -> Result<(), db_error> {
        let localpeer = LocalPeer::new(network_id, parent_network_id, p2p_addr, p2p_port, key_expires, data_url);
    ...
    pub fn connect(path: &String, readwrite: bool, network_id: u32, parent_network_id: u32, key_expires: u64, p2p_addr: PeerAddress, p2p_port: u16, data_url: UrlString, asn4_recs: &Vec<ASEntry4>, initial_neighbors: Option<&Vec<Neighbor>>) -> Result<PeerDB, db_error> {
        ...
        if create_flag {
            ...
            match initial_neighbors {
                Some(ref neighbors) => {
                    db.instantiate(network_id, parent_network_id, key_expires, data_url, p2p_addr, p2p_port, asn4_recs, neighbors)?;
                },
                None => {
                    db.instantiate(network_id, parent_network_id, key_expires, data_url, p2p_addr, p2p_port, asn4_recs, &vec![])?;
                }
            }
        } else {
    ...
    pub fn connect_memory(network_id: u32, parent_network_id: u32, key_expires: u64, data_url: UrlString, asn4_entries: &Vec<ASEntry4>, initial_neighbors: &Vec<Neighbor>) -> Result<PeerDB, db_error> {
        ...
        db.instantiate(network_id, parent_network_id, key_expires, data_url, PeerAddress::from_ipv4(127, 0, 0, 1), NETWORK_P2P_PORT, asn4_entries, initial_neighbors)?;
        Ok(db)
    }
