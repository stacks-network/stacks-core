...
pub mod test {
    ...
    impl TestPeer {
        pub fn new(config: TestPeerConfig) -> TestPeer {
            ...
            let mut peerdb = PeerDB::connect(&peerdb_path, true, config.network_id, config.burnchain.network_id, config.private_key_expire, PeerAddress::from_ipv4(127,0,0,1), NETWORK_P2P_PORT, config.data_url.clone(), &config.asn4_entries, Some(&config.initial_neighbors)).unwrap();
