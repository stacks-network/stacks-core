...
impl TupleData {
    ...
    pub fn deep_merge(base: TupleData, updates: TupleData) -> Result<TupleData> {
        ...
        for (name, value) in updates.data_map.into_iter() {
            let (_, existing_value) = base.data_map.remove_entry(&name)
                .ok_or(CheckErrors::NoSuchTupleField(name.to_string(), base.type_signature.clone()))?;
            
            let new_value = match existing_value {
                Value::Tuple(nested_base) => {
                    let deeply_merged = match value {
                        Value::Tuple(nested_updates) => TupleData::deep_merge(nested_base, nested_updates),
