...
impl<'a, 'b> ReadOnlyChecker<'a, 'b> {
    ...
    fn check_native_function(
        ...
        match function {
            ...
            | IsStandard | ParsePrincipal | BuffToUIntBe | Append | Concat | AsMaxLen
            | ContractOf | PrincipalOf | ListCons | GetBlockInfo | TupleGet | TupleMerge | Len
            | Print | AsContract | Begin | FetchVar | GetStxBalance | StxGetAccount
            | GetTokenBalance | GetAssetOwner | GetTokenSupply | ElementAt | IndexOf => {
                // Check all arguments.
