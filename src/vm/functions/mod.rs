...
pub fn lookup_reserved_functions(name: &str) -> Option<CallableType> {
    ...
    if let Some(native_function) = NativeFunctions::lookup_by_name(name) {
        let callable = match native_function {
            ...
            And => SpecialFunction("native_and", &boolean::special_and),
            Or => SpecialFunction("native_or", &boolean::special_or),
            ...
            If => SpecialFunction("native_if", &special_if),
            Let => SpecialFunction("native_let", &special_let),
            FetchVar => SpecialFunction("native_var-get", &database::special_fetch_variable),
            SetVar => SpecialFunction("native_set-var", &database::special_set_variable),
            Map => SpecialFunction("native_map", &iterables::native_map),
            Filter => SpecialFunction("native_filter", &iterables::native_filter),
            Fold => SpecialFunction("native_fold", &iterables::native_fold),
            ...
            SetEntry => SpecialFunction("native_set-entry", &database::special_set_entry),
            InsertEntry => SpecialFunction("native_insert-entry", &database::special_insert_entry),
            DeleteEntry => SpecialFunction("native_delete-entry", &database::special_delete_entry),
            ...
            Asserts => SpecialFunction("native_asserts", &special_asserts),
