...
pub fn lookup_reserved_functions(name: &str) -> Option<CallableType> {
    ...
    if let Some(native_function) = NativeFunctions::lookup_by_name(name) {
        let callable = match native_function {
            ...
            UnwrapRet => CallableType::NativeFunction("native_unwrap_ret", &options::native_unwrap_or_ret),
            UnwrapErrRet => CallableType::NativeFunction("native_unwrap_err_ret", &options::native_unwrap_err_or_ret),
            ...
            TryRet => CallableType::NativeFunction("native_try_ret", &options::native_try_ret),
