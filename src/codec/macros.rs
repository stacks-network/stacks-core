macro_rules! impl_stacks_message_codec_for_int {
    ($typ:ty; $array:expr) => {
        impl ::codec::StacksMessageCodec for $typ {
            fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), ::codec::Error> {
                fd.write_all(&self.to_be_bytes())
                    .map_err(::codec::Error::WriteError)
            }
            fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<Self, ::codec::Error> {
                let mut buf = $array;
