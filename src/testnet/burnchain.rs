...
impl BurnchainSimulator {
    ...
    pub fn new(config: Config) -> Self {
        let burnchain = Burnchain::new(&config.burnchain.db_path, &config.burnchain.chain, &config.burnchain.mode)
    ...
    pub fn make_genesis_block(&mut self) -> BurnchainState {
        let db = match BurnDB::connect(&self.config.burnchain.db_path, 0, &BurnchainHeaderHash([0u8; 32]), get_epoch_time_secs(), true) {
            Ok(db) => db,
