...
use stacks::chainstate::stacks::boot::{
    COSTS_4_NAME, MINERS_NAME, POX_5_NAME, SIGNERS_VOTING_FUNCTION_NAME, SIGNERS_VOTING_NAME,
...
/// Test PoX-5 activation
///
/// - check epoch 3.5 is active
/// - check pox-5 boot contract exists
#[test]
#[ignore]
#[serial]
fn test_pox_5_activation() {
    ...
    // check for Epoch 3.5 in clarity db
    let latest_stacks_block_id = StacksBlockId::from_hex(
        &test_observer::get_blocks()
            .last()
            .unwrap()
    ...
    assert_eq!(epoch_version, Some(StacksEpochId::Epoch35));

    // check if pox-5 boot contract has been deployed
    let pox_5_boot_contract_exists = chainstate.with_read_only_clarity_tx(
        &sortdb
            .index_handle_at_block(&chainstate, &latest_stacks_block_id)
            .unwrap(),
        &latest_stacks_block_id,
        |conn| {
            conn.with_clarity_db_readonly(|db| {
                db.has_contract(&boot_code_id(POX_5_NAME, naka_conf.is_mainnet()))
            })
        },
    );
