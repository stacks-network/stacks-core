...
#[test]
#[ignore]
/// Verify that the SIP-034 tenure extends are currently disabled in the signer and will be
/// rejected if the miner proposes them.
fn sip034_tenure_extends() {
    if env::var("BITCOIND_TEST") != Ok("1".into()) {
        return;
    }
    ...
    // Mine a regular epoch 3.3 nakamoto tenure
    next_block_and_mine_commit(&mut btc_regtest_controller, 60, &naka_conf, &counters).unwrap();
    ...
    // try to produce a SIP-034 tenure-change, and verify that it gets rejected
    // TODO
