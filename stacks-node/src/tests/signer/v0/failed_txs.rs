...
///
/// Test Setup:
/// - 5 signers, all configured to ignore block proposals (so we control responses).
/// - 2 sender accounts (A and B), each with initial STX balance.
///
/// Test Execution:
/// 1. Boot to epoch 3.
/// 2. Submit 4 transfer txs: sender A nonces 0,1 and sender B nonces 0,1.
/// 3. Wait for the first block proposal containing all 4 transfers.
/// 4. Inject `BlockRejection` messages from all 5 signers with
///    `failed_txid` = sender A's nonce-0 txid and reason `BadTransaction`.
/// 5. Allow signers to handle proposals normally and wait for the miner to
///    repropose and mine a block without sender A's txs.
/// 6. Wait for the miner to propose the next block and verify sender A's
///    txs are included again (the exclusion only lasts one block).
///
/// Test Assertions:
...
fn miner_excludes_failed_txid_and_nonce_dependent_txs() {
    ...
    // Pause mining so we can inject rejections without the miner immediately reproposing after the rejections are received
    TEST_MINE_SKIP.set(true);
    ...
    info!("------------------------- Inject block rejections with failed_txid -------------------------");
    ...
    for (i, signer_private_key) in signer_test.signer_stacks_private_keys.iter().enumerate() {
        // Create a rejection with failed_txid pointing to sender A's nonce-0 tx
    ...
    TEST_MINE_SKIP.set(false);
    info!("------------------------- Wait to mine second block proposal -------------------------");
    // The miner should re-propose after receiving enough rejections.
    // The second proposal should be at the same height but without sender A's txs. (should be accepted)
    ...
    info!("------------------------- Verify second proposal excludes sender A's txs -------------------------");
    let second_txids: Vec<_> = second_block.txs.iter().map(|tx| tx.txid()).collect();
    assert_eq!(
        second_txids.len(),
        2,
        "Second proposal should contain 2 transactions"
    );
    // Sender A's nonce-0 tx was banned (failed_txid)
    assert!(
        !second_txids.contains(&txid_a0),
        "Second proposal must NOT contain sender A tx 0 (it was marked as failed_txid)"
    );
    // Sender A's nonce-1 tx can't be included because nonce 0 is missing
    assert!(
        !second_txids.contains(&txid_a1),
        "Second proposal must NOT contain sender A tx 1 (nonce gap due to excluded nonce 0)"
    );
    // Sender B's txs should still be included
    assert!(
        second_txids.contains(&txid_b0),
        "Second proposal should contain sender B tx 0"
    );
    assert!(
        second_txids.contains(&txid_b1),
        "Second proposal should contain sender B tx 1"
    );
...
/// 3. Wait for the first block proposal containing all 4 transfers.
/// 4. Inject `BlockRejection` messages from all 5 signers with
///    `failed_txid` = sender A's nonce-0 txid and reason
///    `ProblematicTransaction`.
/// 5. Allow signers to handle proposals normally and wait for the miner to
///    repropose and mine a block without sender A's txs.
/// 6. Submit a new transaction from sender B to trigger another block mine and wait for the miner to propose the next block.
/// 7. Verify sender A's txs are still excluded (the ban is permanent — the tx was dropped
///    and blacklisted from the mempool).
///
/// Test Assertions:
/// - The second block excludes sender A's nonce-0 tx (blacklisted) and
///   sender A's nonce-1 tx (can't include due to nonce gap), but includes
///   sender B's 2 transfers.
/// - The third block includes Sender B's new transaction and still excludes sender A's txs (permanent ban, unlike
...
fn miner_permanently_bans_problematic_txid() {
    ...
    let num_signers = 5;
    let sender_a_sk = StacksPrivateKey::from_seed(&[200; 32]);
    let sender_b_sk = StacksPrivateKey::from_seed(&[201; 32]);
    let sender_a_addr = to_addr(&sender_a_sk);
    let sender_b_addr = to_addr(&sender_b_sk);
    ...
    info!("------------------------- Inject block rejections with ProblematicTransaction -------------------------");
    let reward_cycle = signer_test.get_current_reward_cycle();
    let signer_slots = signer_test
        .get_signer_slots(reward_cycle)
        .expect("Failed to get signer slots");
    ...
    for (i, signer_private_key) in signer_test.signer_stacks_private_keys.iter().enumerate() {
        // Create a rejection with failed_txid pointing to sender A's nonce-0 tx
        // using ProblematicTransaction to trigger permanent ban
        let mut rejection = BlockRejection::new(
            proposed_sighash.clone(),
            RejectReason::ValidationFailed(ValidateRejectCode::ProblematicTransaction),
            signer_private_key,
            false, // testnet
            get_epoch_time_secs().saturating_add(u64::MAX),
            get_epoch_time_secs().saturating_add(u64::MAX),
    ...
    // The miner should re-propose after receiving enough rejections.
    // The second block should exclude sender A's txs (blacklisted from mempool).
    let second_block = wait_for_block_pushed_by_miner_key(30, expected_height, &miner_pubk)
        .expect("Miner did not propose a second block");
    info!("------------------------- Verify second block excludes sender A's txs -------------------------");
    let second_txids: Vec<_> = second_block.txs.iter().map(|tx| tx.txid()).collect();
    // Sender A's nonce-0 tx was blacklisted (ProblematicTransaction)
    assert!(
        !second_txids.contains(&txid_a0),
        "Second block must NOT contain sender A tx 0 (blacklisted as problematic)"
    );
    // Sender A's nonce-1 tx can't be included because nonce 0 is missing
    assert!(
        !second_txids.contains(&txid_a1),
        "Second block must NOT contain sender A tx 1 (nonce gap due to blacklisted nonce 0)"
    );
    // Sender B's txs should still be included
    assert!(
        second_txids.contains(&txid_b0),
    ...
    // Sender B: nonce 2
    let tx_b2 =
        make_stacks_transfer_serialized(&sender_b_sk, 2, send_fee, chain_id, &recipient, send_amt);
    let txid_b2_hex =
