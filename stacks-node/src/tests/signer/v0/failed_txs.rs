...
/// same sender) from the next block proposal, but only for the current tenure.
///
/// Test Setup:
...
///    repropose and mine a block.
/// 6. Mine a new tenure and wait for the miner to propose sender A's txs again.
///
/// Test Assertions:
...
/// - After a tenure change, sender A's txs are proposed again (the ban is
///   per-tenure, not permanent) and subsequently accepted.
fn miner_excludes_failed_txid_and_nonce_dependent_txs() {
    ...
    info!("------------------------- Mine a new tenure and mine A's txs -------------------------");
    signer_test.mine_and_verify_confirmed_naka_block(Duration::from_secs(30), num_signers, true);
    // After mining the tenure change, the miner should miner should attempt to propose a block with the 2 sender A txs again, since failed_txid bans the tx only for a given tenure
    let third_block = wait_for_block_pushed_by_miner_key(30, expected_height + 2, &miner_pubk)
        .expect("Miner did not propose a second block");
    let third_txids: Vec<_> = third_block.txs.iter().map(|tx| tx.txid()).collect();
    assert!(
        third_txids.contains(&txid_a0),
        "Third proposal should contain sender A tx 0 again after tenure change (failed_txid bans tx only for a tenure)"
    );
    assert!(
        third_txids.contains(&txid_a1),
        "Third proposal should contain sender A tx 1 again after tenure change (nonce gap should be resolved after including nonce 0)"
    );
    info!("------------------------- Shutdown -------------------------");
