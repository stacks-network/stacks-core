...
fn test_watch_only_wallet_guide_flow_ok() {
    ...
    // guide step 1: watch-only descriptor wallet, loaded on startup
    // (`blank` is irrelevant: a key-less wallet generates no seed anyway)
    client
        .create_wallet("miner", Some(true), Some(true))
