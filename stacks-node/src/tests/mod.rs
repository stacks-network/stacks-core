...
/// If a port previously generated by `gen_random_port` is no longer used, it can be marked as free
/// with this function. It's the caller's responsibility to ensure that the port actually isn't used
/// any more.
pub fn mark_generated_port_as_free(port: u16) {
    let mut ports = USED_PORTS.lock().unwrap();
    if !ports.remove(&port) {
        panic!("trying to mark port as free that was not created by gen_random_port()")
    }
}
