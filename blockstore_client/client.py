...
def get_user_record( name, create=False ):
   ...
   if name_record.has_key('error'):
      
      # failed to look up
      return name_record
   
   # sanity check 
   if not name_record.has_key('value_hash'):
...
def update( name, user_json_or_hash, privatekey, txid=None, proxy=None ):
   ...
   if txid is None:
      ...
      if not result.has_key('transaction_hash'):
...
def namespace_define( namespace_id, lifetime, base_name_cost, cost_decay_rate, privatekey, proxy=None ):
   """
   namesapce_define
   """
   ...
   return proxy.namespace_define( namespace_id, lifetime, base_name_cost, cost_decay_rate, privatekey )
...
def get_mutable( name, data_id, nonce_min=None, nonce_max=None, nonce_check=None ):
   ...
   data = storage.get_mutable_data( data_route, nonce_min=nonce_min, nonce_max=nonce_max, nonce_check=nonce_check )
   if data is None:
...
def put_immutable( name, data, privatekey, txid=None, proxy=None ):
   ...
   if txid is None:
      ...
      if update_result.has_key('error'):
...
def put_mutable( name, data_id, data_text, privatekey, proxy=None, create=True, txid=None, nonce=None, make_nonce=None, replication_strategy=storage.REPLICATE_ALL ):
   """
   put_mutable
   ...
   ** Durability ** 
   ...
   if not user_db.has_mutable_data_route( user, data_id ):
      ...
      route = storage.mutable_data_route( data_id, data_urls, writer_pubkey=writer_pubkey )
...
def delete_mutable( name, data_id, privatekey, proxy=default_proxy, txid=None, route=None ):
   """
   delete_mutable
   """
   ...
   if update_status.has_key('error'):
