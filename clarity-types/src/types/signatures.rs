...
use crate::errors::analysis::{RuntimeAnalysisError, SharedAnalysisError, StaticAnalysisError};
...
impl BufferLength {
    ...
    fn try_from_i128(data: i128) -> Result<Self, SharedAnalysisError> {
        if data > (MAX_VALUE_SIZE as i128) {
            Err(SharedAnalysisError::ValueTooLarge)
        } else if data < 0 {
            Err(SharedAnalysisError::ValueOutOfBounds)
        } else {
...
impl TryFrom<u32> for BufferLength {
    type Error = SharedAnalysisError;
    fn try_from(data: u32) -> Result<BufferLength, SharedAnalysisError> {
...
impl TryFrom<usize> for BufferLength {
    type Error = SharedAnalysisError;
    fn try_from(data: usize) -> Result<BufferLength, SharedAnalysisError> {
        Self::try_from(data as i128)
    }
}
...
impl TryFrom<i128> for BufferLength {
    type Error = SharedAnalysisError;
    fn try_from(data: i128) -> Result<BufferLength, SharedAnalysisError> {
...
impl StringUTF8Length {
    ...
    /// This function is primarily intended for internal runtime use,
    /// and serves as the central place for all integer validation logic.
    fn try_from_i128(value: i128) -> Result<Self, SharedAnalysisError> {
...
impl TryFrom<u32> for StringUTF8Length {
    type Error = SharedAnalysisError;
    fn try_from(data: u32) -> Result<StringUTF8Length, SharedAnalysisError> {
        Self::try_from(data as i128)
    }
}
...
impl TryFrom<usize> for StringUTF8Length {
    type Error = SharedAnalysisError;
    fn try_from(data: usize) -> Result<StringUTF8Length, SharedAnalysisError> {
        Self::try_from(data as i128)
    }
}
...
impl TryFrom<i128> for StringUTF8Length {
    type Error = SharedAnalysisError;
    fn try_from(data: i128) -> Result<StringUTF8Length, SharedAnalysisError> {
        Self::try_from_i128(data)
    }
}
...
impl ListTypeData {
    pub fn new_list(
        ...
        max_len: u32,
    ) -> Result<ListTypeData, SharedAnalysisError> {
        let would_be_depth = 1 + entry_type.depth();
        if would_be_depth > MAX_TYPE_DEPTH {
            return Err(SharedAnalysisError::TypeSignatureTooDeep);
        }
        ...
        let would_be_size = list_data
            .inner_size()?
            .ok_or_else(|| SharedAnalysisError::ValueTooLarge)?;
...
impl TypeSignature {
    pub fn new_option(inner_type: TypeSignature) -> Result<TypeSignature, SharedAnalysisError> {
        ...
        if new_size > MAX_VALUE_SIZE {
            ...
            Err(SharedAnalysisError::TypeSignatureTooDeep)
        } else {
    ...
    ) -> Result<TypeSignature, SharedAnalysisError> {
    ...
    pub fn admits(&self, epoch: &StacksEpochId, x: &Value) -> Result<bool, SharedAnalysisError> {
    ...
    pub fn admits_type(
        ...
        epoch: &StacksEpochId,
        other: &TypeSignature,
    ) -> Result<bool, SharedAnalysisError> {
        match epoch {
            ...
            StacksEpochId::Epoch10 => Err(SharedAnalysisError::Expects(
                "epoch 1.0 not supported".into(),
            )),
        }
    }

    pub fn admits_type_v2_0(&self, other: &TypeSignature) -> Result<bool, SharedAnalysisError> {
        match self {
            ...
            NoType => Err(SharedAnalysisError::CouldNotDetermineType),
            CallableType(_) => Err(SharedAnalysisError::Expects(
                "CallableType should not be used in epoch v2.0".into(),
            )),
            ListUnionType(_) => Err(SharedAnalysisError::Expects(
    ...
    fn admits_type_v2_1(&self, other: &TypeSignature) -> Result<bool, SharedAnalysisError> {
...
impl TryFrom<Vec<(ClarityName, TypeSignature)>> for TupleTypeSignature {
    ...
    fn try_from(
    ...
    ) -> Result<TupleTypeSignature, SharedAnalysisError> {
        if type_data.is_empty() {
            return Err(SharedAnalysisError::EmptyTuplesNotAllowed);
        }
        ...
        for (name, type_info) in type_data.into_iter() {
            if let Entry::Vacant(e) = type_map.entry(name.clone()) {
                ...
                return Err(SharedAnalysisError::NameAlreadyUsed(name.into()));
            }
        }
...
impl TypeSignature {
    ...
    pub fn new_ascii_type(len: i128) -> Result<Self, SharedAnalysisError> {
        ...
        match (a, b) {
            (
                ...
                for (name, entry_a) in types_a.iter() {
                    let entry_b = types_b.get(name).ok_or(SharedAnalysisError::TypeError(
                        Box::new(a.clone()),
                        Box::new(b.clone()),
                    ))?;
                ...
                Ok(TupleTypeSignature::try_from(type_map_out)
                    .map(|x| x.into())
                    .map_err(|_| SharedAnalysisError::SupertypeTooLarge)?)
            }
            (
                SequenceType(SequenceSubtype::ListType(ListTypeData {
                    max_len: len_a,
            ...
            (x, y) => {
                if x == y {
                    ...
                    Err(SharedAnalysisError::TypeError(
                        Box::new(a.clone()),
                        Box::new(b.clone()),
                    ))
                }
    ...
    pub fn type_of(x: &Value) -> Result<TypeSignature, SharedAnalysisError> {
        let out = match x {
    ...
    pub fn parent_list_type(
        ...
        if let Some((first, rest)) = children.split_first() {
            ...
            let len =
                u32::try_from(children.len()).map_err(|_| SharedAnalysisError::ValueTooLarge)?;
...
impl TypeSignature {
    ...
    pub fn size(&self) -> Result<u32, SharedAnalysisError> {
    ...
    fn inner_size(&self) -> Result<Option<u32>, SharedAnalysisError> {
        let out = match self {
            // NoType's may be asked for their size at runtime --
            //  legal constructions like `(ok 1)` have NoType parts (if they have unknown error variant types).
            NoType => Some(1),
    ...
    pub fn type_size(&self) -> Result<u32, SharedAnalysisError> {
