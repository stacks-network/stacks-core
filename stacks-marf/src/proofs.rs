...
use stacks_common::{trace, test_debug, debug, define_u8_enum};
...
use crate::node::{
    ...
    TrieNode, TrieNodeID, TrieNodeType, TriePath,
    TriePtr,
};
use crate::storage::TrieStorageConnection;
use crate::trie::Trie;
use crate::{
    BlockMap, MarfError, MARFValue, MarfTrieId, ProofTrieNode, ProofTriePtr,
    TrieHashExtension, TrieMerkleProof, TrieMerkleProofType,
};
...
impl<T: MarfTrieId> ProofTrieNode<T> {
    ...
    fn try_from_trie_node<N: TrieNode, M: BlockMap>(
        other: &N,
        block_map: &mut M,
    ) -> Result<ProofTrieNode<T>> {
        let id = other.id();
        let path = other.path().clone();
        let ptrs: Result<Vec<_>> = other
            .ptrs()
            .iter()
...
impl<T: MarfTrieId> StacksMessageCodec for ProofTriePtr<T> {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> std::result::Result<(), codec_error> {
        self.id.consensus_serialize(fd)?;
        self.chr.consensus_serialize(fd)?;
        self.back_block.consensus_serialize(fd)
    }

    fn consensus_deserialize<R: Read>(fd: &mut R) -> std::result::Result<ProofTriePtr<T>, codec_error> {
        let id = read_next(fd)?;
...
impl<T: MarfTrieId> StacksMessageCodec for ProofTrieNode<T> {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> std::result::Result<ProofTrieNode<T>, codec_error> {
        let id = read_next(fd)?;
...
impl<T: MarfTrieId> StacksMessageCodec for TrieMerkleProofType<T> {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> std::result::Result<TrieMerkleProofType<T>, codec_error> {
...
impl<Id> StacksMessageCodec for TrieMerkleProof<Id> 
where
    Id: MarfTrieId,
{
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> std::result::Result<(), codec_error> {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> std::result::Result<TrieMerkleProof<Id>, codec_error> {
        let proof_parts: Vec<TrieMerkleProofType<Id>> = read_next(fd)?;
        Ok(TrieMerkleProof(proof_parts))
    }
}

impl<Id> TrieMerkleProof<Id> 
where
    Id: MarfTrieId
{
    ...
    fn ptr_to_segment_proof_node<TrieDB>(
        storage: &mut TrieStorageConnection<Id, TrieDB>,
        ptr: &TriePtr,
        prev_chr: u8,
    ) -> Result<TrieMerkleProofType<Id>> 
    where
        TrieDB: TrieDb
    {
        trace!(
        ...
        let hashes = if node.is_leaf() {
            ...
            TrieMerkleProof::<Id>::make_proof_hashes(&node, &all_hashes, prev_chr)?
        };
    ...
    fn make_initial_shunt_proof<TrieDB>(
        storage: &mut TrieStorageConnection<Id, TrieDB>,
    ) -> Result<Vec<TrieMerkleProofType<Id>>> 
    where
        TrieDB: TrieDb
    {
    ...
    fn make_backptr_shunt_proof<TrieDB>(
        storage: &mut TrieStorageConnection<Id, TrieDB>,
        backptr: &TriePtr,
    ) -> Result<Vec<TrieMerkleProofType<Id>>> 
    where
    ...
    fn verify_shunt_proof_head(
        ...
        shunt_proof_head: &TrieMerkleProofType<Id>,
    ) -> Option<TrieHash> {
    ...
    fn verify_shunt_proof_tail(
        ...
        shunt_proof: &[TrieMerkleProofType<Id>],
    ) -> Option<TrieHash> {
        ...
        for i in 0..shunt_proof.len() {
            ...
            hash = match proof_node {
                TrieMerkleProofType::Shunt((ref idx, ref hashes)) => {
                    ...
                    match TrieMerkleProof::<Id>::next_shunt_hash(&hash, *idx, hashes) {
                        Some(h) => h,
                        None => {
                            return None;
                        }
    ...
    fn verify_shunt_proof_junction(
        ...
        shunt_proof_junction: &TrieMerkleProofType<Id>,
    ) -> Option<TrieHash> {
    ...
    fn get_segment_proof_path_prefix(segment_proof: &[TrieMerkleProofType<Id>]) -> Option<Vec<u8>> {
    ...
    fn is_proof_well_formed(proof: &Vec<TrieMerkleProofType<Id>>, expected_path: &TriePath) -> bool {
        if proof.len() == 0 {
    ...
    pub fn verify_proof(
        proof: &Vec<TrieMerkleProofType<Id>>,
        path: &TriePath,
        value: &MARFValue,
        root_hash: &TrieHash,
        root_to_block: &HashMap<TrieHash, Id>,
    ) -> bool {
    ...
    pub fn verify(
        ...
        TrieMerkleProof::<Id>::verify_proof(&self.0, &path, &marf_value, root_hash, root_to_block)
    }

    /// Walk down the trie pointed to by s until we reach a backptr or a leaf
    fn walk_to_leaf_or_backptr<TrieDB>(
        storage: &mut TrieStorageConnection<Id, TrieDB>,
        path: &TriePath,
    ) -> Result<(TrieCursor<Id>, TrieNodeType, TriePtr)> 
    where
        TrieDB: TrieDb
    {
        trace!(
    ...
    pub fn from_path<TrieDB>(
        storage: &mut TrieStorageConnection<Id, TrieDB>,
        path: &TriePath,
        expected_value: &MARFValue,
        root_block_header: &Id,
    ) -> Result<TrieMerkleProof<Id>> 
    where
        TrieDB: TrieDb
    {
