...
impl TrieFile {
    ...
    fn new_disk(path: &str, readonly: bool) -> Result<TrieFile> {
    ...
    /// Return the trie ID
    pub fn store_trie_blob<Id, TrieDB>(
        &mut self,
        db: &TrieDB,
        bhh: &Id,
        buffer: &[u8],
    ...
    fn read_trie_blob_from_db<TrieDB>(
        db: &TrieDB, 
        block_id: u32
    ) -> Result<Vec<u8>> 
    where
    ...
    {
        let trie_blob = {
            let mut fd = db.open_trie_blob_readonly(block_id)?;
            let mut cursor = Cursor::new(fd);
            let mut trie_blob = vec![];
            cursor.read_to_end(&mut trie_blob)?;
            trie_blob
        };
        Ok(trie_blob)
    }
...
pub struct TrieFileNodeHashReader<'a, TrieDB> 
where
    TrieDB: TrieDb
{
    db: &'a TrieDB,
...
impl<'a, TrieDB> TrieFileNodeHashReader<'a, TrieDB> 
where
    TrieDB: TrieDb
{
    pub fn new(
        ...
        file: &'a mut TrieFile,
        block_id: u32,
    ) -> TrieFileNodeHashReader<'a, TrieDB> {
        TrieFileNodeHashReader { db, file, block_id }
    }
}

impl<TrieDB> NodeHashReader for TrieFileNodeHashReader<'_, TrieDB> 
where
    TrieDB: TrieDb
{
...
impl TrieFile {
    ...
    pub fn get_node_hash_bytes<TrieDB>(
        &mut self,
        db: &TrieDB,
        block_id: u32,
        ptr: &TriePtr,
    ...
    pub fn read_node_type_nohash<TrieDB>(
        &mut self,
        db: &TrieDB,
        block_id: u32,
        ptr: &TriePtr,
        ...
        TrieDB: TrieDb
    {
        ...
        let rows = s.query_and_then(NO_PARAMS, |row| {
            let block_hash: Id = row.get_unwrap("block_hash");
            let offset_i64: i64 = row.get_unwrap("external_offset");
            let offset = offset_i64 as u64;
            let start = TrieStorageConnection::<Id, TrieDB>::root_ptr_disk() as u64;
    ...
    pub fn append_trie_blob<TrieDB>(
        &mut self, 
        db: &TrieDB, 
        buf: &[u8]
    ) -> Result<u64> 
