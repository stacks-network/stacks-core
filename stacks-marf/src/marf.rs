...
use stacks_common::{error, debug, trace, test_debug, warn, info};
...
use crate::trie::Trie;
use crate::{
    MarfError, MARFValue, MarfTrieId, TrieHashExtension, TrieLeaf, TrieMerkleProof,
};
...
pub struct MARF<Id, TrieDB> 
where
    Id: MarfTrieId,
    TrieDB: TrieDb
{
    storage: TrieFileStorage<Id, TrieDB>,
    open_chain_tip: Option<WriteChainTip<Id>>,
}

pub struct MarfTransaction<'a, Id, TrieDB> 
where
    Id: MarfTrieId,
    TrieDB: TrieDb
{
...
///
pub trait MarfConnection<Id, TrieDB> 
where
    Id: MarfTrieId,
    TrieDB: TrieDb
{
    ...
    where
        F: FnOnce(&mut TrieStorageConnection<Id, TrieDB>) -> R;

    /// Resolve a key from the MARF to a MARFValue with respect to the given block height.
    fn get(&mut self, block_hash: &Id, key: &str) -> Result<Option<MARFValue>, MarfError> {
        self.with_conn(|c| MARF::<Id, TrieDB>::get_by_key(c, block_hash, key))
    }
    ...
    fn get_with_proof(
        ...
        block_hash: &Id,
        key: &str,
    ) -> Result<Option<(MARFValue, TrieMerkleProof<Id>)>, MarfError> {
        self.with_conn(|conn| {
            let marf_value = match MARF::<Id, TrieDB>::get_by_key(conn, block_hash, key)? {
                None => return Ok(None),
                Some(x) => x,
            };
    ...
    fn get_block_at_height(&mut self, height: u32, tip: &Id) -> Result<Option<Id>, MarfError> {
        self.with_conn(|c| MARF::<Id, TrieDB>::get_block_at_height(c, height, tip))
    }

    fn get_block_height(&mut self, ancestor: &Id, tip: &Id) -> Result<Option<u32>, MarfError> {
        self.with_conn(|c| MARF::<Id, TrieDB>::get_block_height(c, ancestor, tip))
    }

    /// Get the root trie hash at a particular block
    fn get_root_hash_at(&mut self, block_hash: &Id) -> Result<TrieHash, MarfError> {
    ...
    fn check_ancestor_block_hash(&mut self, bhh: &Id) -> Result<(), MarfError> {
        self.with_conn(|conn| {
...
impl<'a, Id, TrieDB> MarfConnection<Id, TrieDB> for MarfTransaction<'a, Id, TrieDB> 
where
    Id: MarfTrieId,
    TrieDB: TrieDb
{
...
impl<TrieDB, Id> MarfConnection<Id, TrieDB> for MARF<Id, TrieDB> 
where
    Id: MarfTrieId,
    TrieDB: TrieDb
{
...
///   aborted
///
impl<'a, Id, TrieDB> MarfTransaction<'a, Id, TrieDB>
where
    Id: MarfTrieId,
    TrieDB: TrieDb
{
    ...
    pub fn commit_to(mut self, real_bhh: &Id) -> Result<(), MarfError> {
        if self.storage.readonly() {
            return Err(MarfError::ReadOnlyError);
        }
    ...
    pub fn commit_mined(mut self, bhh: &Id) -> Result<(), MarfError> {
        if self.storage.readonly() {
            return Err(MarfError::ReadOnlyError);
        }
        if self.storage.unconfirmed() {
    ...
    pub fn get_open_chain_tip(&self) -> Option<&Id> {
    ...
    pub fn get_block_height_of(
        ...
        if Some(bhh) == self.get_open_chain_tip() {
            ...
            MARF::<Id, TrieDB>::get_block_height_miner_tip(&mut self.storage, bhh, current_block_hash)
        }
    }
    ...
    fn inner_get_extension_height(
        ...
        let is_parent_sentinel = chain_tip == &Id::sentinel();
    ...
    pub fn begin_unconfirmed(&mut self, chain_tip: &Id) -> Result<Id, MarfError> {
        ...
        let unconfirmed_tip = MARF::<Id, TrieDB>::make_unconfirmed_chain_tip(chain_tip);
    ...
    TrieDB: TrieDb
{
    ...
    fn walk_backptr(
        storage: &mut TrieStorageConnection<Id, TrieDB>,
        start_node: &TrieNodeType,
        chr: u8,
        cursor: &mut TrieCursor<Id>,
    ...
    fn node_copy_update(node: &mut TrieNodeType, child_block_id: u32) -> Result<TrieHash, MarfError> {
        let hash = match node {
            ...
            _ => {
                MARF::<Id, TrieDB>::node_copy_update_ptrs(node.ptrs_mut(), child_block_id);
                TrieHash::from_data(&[])
            }
        };
    ...
    fn node_child_copy(
        ...
        chr: u8,
        cursor: &mut TrieCursor<Id>,
    ) -> Result<(TrieNodeType, TrieHash, TriePtr, Id), MarfError> {
        trace!(
            "Copy to {:?} child {:x} of {:?}",
        ...
        let child_hash = MARF::<Id, TrieDB>::node_copy_update(&mut child_node, child_block_identifier)
    ...
    fn root_copy(storage: &mut TrieStorageConnection<Id, TrieDB>, prev_block_hash: &Id) -> Result<(), MarfError> {
        let (cur_block_hash, cur_block_id) = storage.get_cur_block_and_id();
        ...
        let new_root_hash = MARF::<Id, TrieDB>::node_copy_update(&mut prev_root, prev_block_identifier)?;
    ...
    pub fn extend_trie<'a>(
        ...
        if storage.num_blocks() == 0 || cur_bhh == Id::sentinel() {
            // brand new storage
    ...
    fn walk(
        ...
        block_hash: &Id,
        path: &TriePath,
    ) -> Result<(TrieCursor<Id>, TrieNodeType), MarfError> {
    ...
    pub fn from_storage(storage: TrieFileStorage<Id, TrieDB>) -> MARF<Id, TrieDB> {
        MARF {
            storage,
            open_chain_tip: None,
        }
    ...
    pub fn from_path(path: &str, open_opts: MARFOpenOpts) -> Result<MARF<Id, TrieDB>, MarfError> {
    ...
    /// This will have the side-effect of instantiating a new fork table from the tries encoded on
    /// disk. Performant code should call this method sparingly.
    pub fn from_path_unconfirmed(path: &str, open_opts: MARFOpenOpts) -> Result<MARF<Id, TrieDB>, MarfError> {
    ...
    */
    ...
    pub fn get_block_at_height(
        ...
        MARF::get_by_key(storage, current_block_hash, &height_key)
            .map(|option_result| option_result.map(Id::from))
    }
    ...
    pub fn make_unconfirmed_chain_tip(chain_tip: &Id) -> Id {
...
where
    ...
    TrieDB: TrieDb
{
    pub fn begin_tx(&mut self) -> Result<MarfTransaction<Id, TrieDB>, MarfError> {
    ...
    pub fn open_block(&mut self, block_hash: &Id) -> Result<(), MarfError> {
    ...
    pub fn get_bhh_at_height(&mut self, block_hash: &Id, height: u32) -> Result<Option<Id>, MarfError> {
    ...
    /// Drop the current trie from the MARF, and roll back all unconfirmed state
    pub fn drop_unconfirmed(&mut self) {
        if !self.storage.is_readonly() && self.storage.unconfirmed() {
            if let Some(tip) = self.open_chain_tip.take() {
                let mut tx = self
                    .storage
                    .transaction()
    ...
    pub fn reopen_storage_readonly(&self) -> Result<TrieFileStorage<Id, TrieDB>, MarfError> {
    ...
    /// Get the root trie hash at a particular block
    pub fn get_root_hash_at(&mut self, block_hash: &Id) -> Result<TrieHash, MarfError> {
