...
pub use storage::TrieFileStorage;
pub use marf::{MARF, MARFOpenOpts};
pub use errors::MarfError;
pub use db::*;
...
pub type Result<T> = std::result::Result<T, MarfError>;
...
pub trait BlockMap {
    ...
    fn get_block_hash(&self, id: u32) -> Result<Self::TrieId>;
    fn get_block_hash_caching(&mut self, id: u32) -> Result<&Self::TrieId>;
...
impl BlockMap for () {
    ...
    fn get_block_hash_caching(&mut self, _id: u32) -> Result<&BlockHeaderHash> {
        Err(MarfError::NotFoundError)
    }
    fn is_block_hash_cached(&self, _id: u32) -> bool {
        false
    }
    fn get_block_id(&self, _bhh: &BlockHeaderHash) -> Result<u32> {
        Err(MarfError::NotFoundError)
    }
    fn get_block_id_caching(&mut self, _bhh: &BlockHeaderHash) -> Result<u32> {
        Err(MarfError::NotFoundError)
    }
}
