...
export class StackAggregationCommitIndexedAuthCommand_Err
  implements PoxCommand {
  ...
  /**
   * Constructs a `StackAggregationCommitIndexedAuthCommand_Err` to commit partially 
   * locked uSTX.
   *
  ...
  run(model: Stub, real: Real): void {
    ...
    expect(block[1].result).toBeErr(
      Cl.int(this.errorCode),
    );
