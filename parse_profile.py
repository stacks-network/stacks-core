...
def process_events(events):
    ...
    for event in events:
        ...
        # Q6
            ...
            block_processing_times.append(elapsed)
        # Q7
            ...
            if node_startup_time == None and node_start_time != None:
                node_startup_time = event["details"]["timestamp"] - node_start_time
        # Q8
...
def compute_q3(tx_cost_data, block_limit):
...
def compute_q4(block_tx_data, min_burn_height=None):
...
#  "parent_tip_height": _,
#  "block_limit_hit": _ }
def compute_q5a(block_limit_hit_data, total_blocks_mined):
...
def compute_q6(timestamps):
    ...
    print("50th percentile:", avg)
    print("95th percentile:", nf_perc)
    print("Num values:", len(timestamps))

# Q7: No logging - how long does it take to start a non-sidecar follower from genesis?
def compute_q7(node_startup_time):
    print("\nAnswering Q7: amount of time (in secs) to start a node from genesis")
    print("Elapsed time (secs): ", node_startup_time)

# Q8: How long does it take a miner to assemble a block?
def compute_q8(mining_times):
...
def compute_stats_from_log_file(file_name="sample_logs.txt"):
    ...
    compute_q1_and_q2(block_tenure_stats)
    compute_q3(tx_costs, block_limit)
    compute_q4(block_tx_data)
    compute_q5a(anchored_block_limit_hit, len(mining_times))
    compute_q5b(microblock_limit_hit)
    compute_q6(block_processing_times)
    compute_q7(node_startup_time)
    compute_q8(mining_times)
