...
These instructions define pull-request review criteria, not code-generation requirements. Review the proposed diff and enough surrounding code to validate it. Do not recommend optional rewrites, style preferences, or unrelated cleanup unless they address a concrete risk introduced or exposed by the PR.
...
- Check required node and signer changelog fragments or no-changelog handling, including both components when the PR affects them independently.
