...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=80.0,
        type=float,
        help="Per-test percentile across input runs (default: 80)",
    )
    ...
    if not 0 < args.test_percentile <= 100:
        parser.error("--test-percentile must be in (0, 100]")
...
def discover_junit_groups(
    input_dirs: list[Path], junit_files: list[Path]
) -> list[list[Path]]:
    """Return one deterministic JUnit-file group per sampled CI run."""
    groups: list[list[Path]] = []
    for input_dir in input_dirs:
        if not input_dir.is_dir():
            raise ValueError(f"JUnit input directory not found: {input_dir}")
        discovered = sorted(path.resolve() for path in input_dir.rglob("junit*.xml"))
        if not discovered:
            raise ValueError(f"no junit*.xml files found in {input_dir}")
        groups.append(discovered)

    if junit_files:
        groups.append(sorted(path.resolve() for path in set(junit_files)))
...
def aggregate_run_durations(
    run_durations: list[dict[str, float]], test_percentile: float
) -> dict[str, float]:
    """Aggregate per-test durations across sampled runs."""
    test_samples: defaultdict[str, list[float]] = defaultdict(list)
    for durations in run_durations:
        for name, duration in durations.items():
            test_samples[name].append(duration)
    return {
        name: nearest_rank_percentile(samples, test_percentile)
...
def main() -> None:
    ...
    args = parse_args()
    try:
        junit_groups = discover_junit_groups(args.input_dir, args.junit_files)
        run_durations = [collect_test_durations(group) for group in junit_groups]
        durations = aggregate_run_durations(run_durations, args.test_percentile)
