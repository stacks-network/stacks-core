...
if virtualchain.version_byte == 0:
    # mainnet
...
elif virtualchain.version_byte == 111:
    # testnet
    ...
    raise Exception("Unknown version byte %s" % virtualchain.version_byte)
...
class BlockHeaderClient( BitcoinBasicClient ):
    ...
    def handle_headers( self, message_header, block_headers_message ):
        ...
        if current_height >= 0:
            ...
            log.debug("Receive %s testnet headers (%s to %s)" % (len(block_headers), current_height + 1, current_height + 1 + len(block_headers)))
            last_header = {
