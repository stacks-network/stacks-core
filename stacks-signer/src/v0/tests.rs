...
mod async_sibling_validation {
    ...
    fn run_sibling_scenario(
        ...
        let block_a = tenure_start(&miner, &tenure, &parent_tenure, &parent_id, 10, now);
        let block_b = tenure_start(&miner, &tenure, &parent_tenure, &parent_id, 10, now + 1);
        ...
        if permit_reorg_of_tenure {
            ...
            node.signer
                .signer_db
                .mark_tenure_superseded(&tenure, 1, &permitting_tenure, &permitting_burn_hash)
                .unwrap();
