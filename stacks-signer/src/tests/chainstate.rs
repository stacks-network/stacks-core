...
fn check_proposal_units() {
    ...
    sortitions_view.signer_state.current_miner = MinerState::ActiveMiner {
        current_miner_pkh: current_sortition.miner_pkh,
        tenure_id: block.header.consensus_hash,
        parent_tenure_id: current_sortition.parent_tenure_id,
...
fn reorg_timing_testing(
    ...
    signer_db
        .insert_burn_block(
            &cur_sortition.burn_block_hash,
            &cur_sortition.consensus_hash,
            3,
            &sortition_time,
            &last_sortition.burn_block_hash,
        )
        .unwrap();
    ...
    let h = std::thread::spawn(move || {
        cur_sortition.check_parent_tenure_choice(
            &signer_db,
            &client,
            &sortitions_view.config.first_proposal_burn_block_timing,
        )
...
fn check_parent_tenure_choice_reorg_timing_bad() {
    let is_good = reorg_timing_testing("reorg_timing_bad", 30, 31).unwrap();
...
fn check_parent_tenure_choice_reorg_timing_ok() {
    let is_good = reorg_timing_testing("reorg_timing_okay", 30, 29).unwrap();
...
fn check_sortition_timeout() {
    ...
    assert!(
        !SortitionState::is_timed_out(&consensus_hash, &signer_db, Duration::from_secs(10))
            .unwrap()
    );
    // We are a valid sortition, have an empty tenure, and have now timed out
    assert!(
        SortitionState::is_timed_out(&consensus_hash, &signer_db, Duration::from_secs(1)).unwrap()
    );
