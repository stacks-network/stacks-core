...
pub(crate) mod tests {
    ...
    pub fn build_get_signers_response(config: &Config) -> (String, Vec<StacksAddress>) {
        let (_generated_public_keys, _signer_key_ids, stacks_addresses) = generate_public_keys(
            10,
            4000,
            Some(
    ...
    pub fn generate_public_keys(
    ...
    ) -> (PublicKeys, HashMap<u32, HashSet<u32>>, Vec<StacksAddress>) {
        assert!(
            num_signers > 0,
        ...
        (public_keys, signer_key_ids, addresses)
    }
}
