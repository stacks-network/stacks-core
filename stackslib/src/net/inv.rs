...
impl InvState {
    ...
    where
        SortDB: SortitionDb
    {
        let sn = match sortdb.get_block_snapshot_consensus(&consensus_hash)? {
            Some(sn) => {
                if !sn.pox_valid {
                    debug!(
    ...
    pub fn set_block_available<SortDB>(
        &mut self,
        burnchain: &Burnchain,
        neighbor_key: &NeighborKey,
        sortdb: &SortDB,
    ...
    pub fn set_microblocks_available<SortDB>(
        &mut self,
        burnchain: &Burnchain,
        neighbor_key: &NeighborKey,
        sortdb: &SortDB,
...
impl PeerNetwork {
    ...
    fn get_tip_sortition_snapshot<SortDB>(
        &self, 
        sortdb: &SortDB
    ...
    where
        SortDB: SortitionDb
    {
        match sortdb.get_block_snapshot(&self.tip_sort_id)? {
            Some(sn) => {
                if !sn.pox_valid {
    ...
    pub fn get_ancestor_sortition_snapshot<SortDB>(
        &self,
        sortdb: &SortDB,
        height: u64,
    ) -> Result<BlockSnapshot, net_error> 
    ...
    fn get_peer_sortition_snapshot<SortDB>(
        &self,
        sortdb: &SortDB,
    ...
    fn get_getblocksinv_num_blocks<SortDB>(
        &self,
        sortdb: &SortDB,
        target_block_reward_cycle: u64,
        nk: &NeighborKey,
    ...
    fn make_next_getblocksinv<SortDB>(
        &self,
        sortdb: &SortDB,
        nk: &NeighborKey,
        stats: &NeighborBlockStats,
    ...
    fn get_block_scan_start<SortDB>(
        &self, 
        sortdb: &SortDB, 
        ...
        SortDB: SortitionDb
    {
        ...
        let (consensus_hash, _) = sortdb.get_canonical_stacks_chain_tip_hash()
    ...
    /// Return true if done.
    fn inv_getpoxinv_try_finish<SortDB>(
        &mut self,
        sortdb: &SortDB,
        nk: &NeighborKey,
        stats: &mut NeighborBlockStats,
    ...
    fn inv_getblocksinv_begin<SortDB>(
        &mut self,
        sortdb: &SortDB,
        nk: &NeighborKey,
        stats: &mut NeighborBlockStats,
    ...
    pub fn refresh_sortition_view<SortDB>(
        &mut self, 
        sortdb: &SortDB
    ) -> Result<(), net_error> 
    where
    ...
    pub fn sync_inventories<SortDB>(
        &mut self,
        sortdb: &SortDB,
        ibd: bool,
        // TODO: Use a return type
    ...
    pub fn get_local_blocks_inv<SortDB, ChainDB>(
        &mut self,
        sortdb: &SortDB,
...
mod test {
    ...
    fn test_sync_inv_set_blocks_microblocks_available() {
        ...
        let first_stacks_block_height = {
            let sn =
                peer_1.sortdb.get_canonical_burn_chain_tip()
                    .unwrap();
            sn.block_height
        };
    ...
    fn test_sync_inv_make_inv_messages() {
        ...
        let mut peer_1 = 
            TestPeer::new(peer_1_config);

        let first_stacks_block_height = {
            let sn =
                peer_1.get_canonical_burn_chain_tip()
                    .unwrap();
            sn.block_height
        };
        ...
        let getpoxinv_request = peer_1
            .with_network_state(|sortdb, _chainstate, network, _relayer, _mempool| {
                ...
                let sn = {
                    let ic = sortdb.index_conn();
                    let sn = sortdb.get_ancestor_snapshot(&ic, height, &tip.sortition_id)
                        .unwrap()
                        .unwrap();
                    sn
                };
