                ...
                chainstate.maybe_read_only_clarity_tx(
                    ...
                    |clarity_tx| {
                        ...
                        clarity_tx.with_readonly_clarity_env(
                            ...
                            cost_track,
                            |env| {
                                // we want to execute any function as long as no actual writes are
                                // made as opposed to be limited to
                                // purely calling `define-read-only` functions,
                                // so use `read_only = false`.  This broadens the number of
                                // functions that can be called, and
                                // also circumvents limitations on `define-read-only`
                                // functions that can not use `contrac-call?`, even when calling
