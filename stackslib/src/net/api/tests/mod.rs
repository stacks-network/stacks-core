...
/// This helper function drives I/O between a sender and receiver Http
/// conversation.
...
pub struct TestRPC<'a> {
    ...
    /// whether or not to maintain unconfirmed microblocks (e.g. this is false
    /// for nakamoto)
...
impl<'a> TestRPC<'a> {
    ...
    pub fn setup_ex(test_name: &str, process_microblock: bool) -> TestRPC<'a> {
        ...
        // stacker DBs get initialized thru reconfiguration when the above block gets
        // processed
        ...
        // build 1-block microblock stream with the contract-call and the unconfirmed
        // contract
        let microblock = {
...
/// This function sets up two TestPeers and their respective chainstates, and
/// loads them up with some sample blocks and microblocks.  The blocks will
/// contain a smart contract transaction called `hello-world` with the code
/// `TEST_CONTRACT` above.  In addition, a microblock will be created off of the
/// block with a contract-call to `add-unit`.  The second TestPeer will also
/// have a populated mempool, while the first will not.
///
/// This function causes the first peer to send `request` to the second peer
/// from the first peer, and will return the `StacksHttpResponse` generated by
/// the second peer.
