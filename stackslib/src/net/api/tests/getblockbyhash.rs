...
fn test_try_parse_request() {
    ...
    let request =
        StacksHttpRequest::new_get_nakamoto_block_by_hash(addr.into(), BlockHeaderHash([0x11; 32]));
    let bytes = request.try_serialize().unwrap();

    debug!("Request:\n{}\n", std::str::from_utf8(&bytes).unwrap());

    let (parsed_preamble, offset) = http.read_preamble(&bytes).unwrap();
    let mut handler = getblockbyhash::RPCNakamotoBlockByHashRequestHandler::new();
    ...
    // consumed path args
    assert_eq!(handler.block_hash, Some(BlockHeaderHash([0x11; 32])));

    assert_eq!(&preamble, request.preamble());

    handler.restart();
    assert!(handler.block_hash.is_none());
}
...
fn test_try_make_response() {
    ...
    let nakamoto_chain_tip_hash = rpc_test.tip_hash.clone();
