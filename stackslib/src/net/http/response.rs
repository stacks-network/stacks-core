...
/// HTTP response preamble.  This captures all HTTP header information, but in a
/// way that certain fields that nodes rely on are guaranteed to have correct,
/// sensible values. The code calls this a "preamble" to be consistent with the
/// Stacks protocol family system.
...
pub struct HttpResponsePreamble {
    ...
    pub keep_alive: bool,
    /// Content-Length value, if given.  If it's not given, then the payload
    /// will be treated as chunk-encoded (and it had better have a
...
/// HTTP response body generated by the request handler.  It implements a means
/// of streaming data from disk or RAM into a socket buffer as space within it
/// frees up.  Use one of the constructors below to generate the response
/// contents.
...
impl HttpResponsePayload {
    ...
    /// This is here for the times where you've already sent the HTTP preamble
    /// while designating chunked-enoding, but you don't (yet) have a
    /// Streamer implementation for your body.
    ///
    /// You really should not use this in production.  It's mainly used in
    /// testing clients who use this library.
