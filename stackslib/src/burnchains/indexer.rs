...
pub trait BurnchainIndexer {
    ...
    /// Return headers that fall within the range. If end_block extends beyond
    /// the downloaded header range, then the result is truncated.
    ...
    /// Make an instance of the indexer to be consumed by a burnchain indexer
    /// thread, for reading local state (but not downloading or parsing it).
    /// This is different from `clone()` in that not all state needs to be
    /// copied.
    fn reader(&self) -> Self;
}
