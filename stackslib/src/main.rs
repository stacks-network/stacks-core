...
",
    ...
    if argv[1] == "replay-block" {
        ...
        let mut stmt = if argv.len() == 4 {
            ...
            let start_height: u64 = argv[3].parse().unwrap();
            let num_blocks: u64 = argv[4].parse().unwrap();
            let end_height = start_height + num_blocks;
            println!("Will check blocks {} to {}...", start_height, end_height);
            conn.prepare(&format!(
                "SELECT index_block_hash FROM staging_blocks WHERE height >= {} AND height < {}",
