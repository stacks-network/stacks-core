...
impl NakamotoBlock {
    ...
    /// * It has exactly one TenureChange, and it does _not_ require a sortition (it's `cause` is
    /// `Extended`)
    ...
    pub(crate) fn check_coinbase_tx(
        ...
        // parent tenure's VRF proof via the `validate_vrf_seed()` method, which requires that we
...
impl NakamotoChainState {
    ...
    pub fn validate_nakamoto_block_burnchain(
        ...
        // miner began its tenure; it may not be the burnchain tip.
