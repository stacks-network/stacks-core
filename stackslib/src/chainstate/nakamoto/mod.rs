...
impl NakamotoBlock {
    ...
    ///
    /// If this is a shadow block, then the coinbase recipient must be the burn address.  The VRF
    /// proof will only be checked to see that it is present, since this is required by epoch 3.x.
    pub(crate) fn check_coinbase_tx(
        &self,
        mainnet: bool,
        leader_vrf_key: Option<&VRFPublicKey>,
        ...
        if let Some(coinbase_tx) = self.get_coinbase_tx() {
            ...
            if self.is_shadow_block() {
                ...
                let Some(leader_vrf_key) = leader_vrf_key else {
                    warn!(
                        "Invalid Stacks block header {}: no VRF key given and not a shadow block",
                        &self.header.block_hash()
                    );
                    return Err(ChainstateError::InvalidStacksBlock(
                        "No VRF key found".into(),
                    ));
                };
                ...
                if !valid {
                    ...
                    return Err(ChainstateError::InvalidStacksBlock(
                        "Invalid Nakamoto block: leader VRF key did not produce a valid proof"
                            .into(),
                    ));
                }
            }
    ...
    pub fn validate_against_burnchain(
        ...
        miner_pubkey_hash160: Option<&Hash160>,
        ...
        if !self.is_shadow_block() {
            let Some(miner_pubkey_hash160) = miner_pubkey_hash160 else {
                warn!("No miner key available and not a shadow block");
...
impl NakamotoChainState {
    ...
    pub fn validate_nakamoto_block_burnchain(
        ...
        // leader key of the miner
        let leader_key_opt = if block.is_shadow_block() {
            // if this is a shadow block, then this sortition has no winner.
            None
        } else {
        ...
        // miner key hash160.
        // must be defined if this is not a shadow block (checked in validate_against_burnchain())
        let miner_pubkey_hash160_opt = leader_key_opt
            .as_ref()
            .map(|lk| {
                lk.interpret_nakamoto_signing_key()
        ...
        // miner VRF public key
        // must be defined if this is not a shadow block (checked in validate_against_burnchain())
        let vrf_public_key_opt = leader_key_opt.as_ref().map(|lk| &lk.public_key);
        ...
        if let Err(e) = block.validate_against_burnchain(
            ...
            expected_burn,
            miner_pubkey_hash160_opt.as_ref(),
            vrf_public_key_opt,
        ) {
            warn!(
    ...
    pub fn get_parent_vrf_proof<SDBI: StacksDBIndexed>(
            ...
            NakamotoChainState::get_nakamoto_block_version(chainstate_conn.sqlite(), tip_block_id)?
        {
            if NakamotoBlockHeader::is_shadow_block_version(parent_version) {
                let parent_tenure_consensus_hash =
                    NakamotoChainState::get_block_header_nakamoto_tenure_id(
                        chainstate_conn.sqlite(),
                        tip_block_id,
                    )?
                    .ok_or_else(|| {
                ...
                // the parent tenure won't have a block-commit, but we just found its tenure ID anyway
                debug!(
                    "Load VRF proof for shadow tenure {}",
                    &parent_tenure_consensus_hash
                );
                let parent_vrf_proof = Self::get_block_vrf_proof(
                    chainstate_conn,
                    tip_block_id,
                    &parent_tenure_consensus_hash,
                )?
                .ok_or_else(|| {
                    warn!("No VRF proof for {}", &parent_tenure_consensus_hash);
                    ChainstateError::NoSuchBlockError
                })
                .map_err(|e| {
                    warn!("Could not find shadow tenure parent VRF proof";
                              "tip_block_id" => %tip_block_id,
                              "parent consensus_hash" => %parent_tenure_consensus_hash,
                              "block consensus_hash" => %consensus_hash);
                    e
                })?;

                return Ok(parent_vrf_proof);
            }
        }
    ...
    /// * tenure_block_commit_opt: the block commit that elected this miner. This is required unless
    /// the block is a shadow block (not checked here).
    ...
    pub fn setup_block<'a, 'b>(
        ...
        tenure_block_commit_opt: Option<&LeaderBlockCommitOp>,
        active_reward_set: &RewardSet,
    ) -> Result<SetupBlockResult<'a, 'b>, ChainstateError> {
        if let Some(tenure_block_commit) = tenure_block_commit_opt {
    ...
    > {
        ...
        let tenure_block_commit_opt = if !block.is_shadow_block() {
            ...
            // shadow blocks aren't bound to sortitions
            None
        };
        ...
        if new_tenure && parent_chain_tip.is_nakamoto_block() && !block.is_first_mined() {
            ...
            if let Some(tenure_block_commit) = tenure_block_commit_opt.as_ref() {
                ...
                // this should be unreachable, but we have it here defensively
                if !block.is_shadow_block() {
                    warn!("Invalid Nakamoto block: no tenure block-commit found (and not a shadow block)");
                    return Err(ChainstateError::NoSuchBlockError);
                }
            }
        }
        ...
        let SetupBlockResult {
            ...
            tenure_block_commit_opt.as_ref(),
            active_reward_set,
        )?;
