...
mod pox_5_admin_tests {
    ...
    fn non_mainnet_resolution() {
        ...
        set_pox_5_bond_admin(Some(override_principal.clone()));
        set_pox_5_pause_admin(Some(override_principal.clone()));
        assert_eq!(pox_5_bond_admin(false), override_principal);
        assert_eq!(pox_5_pause_admin(false), override_principal);

        set_pox_5_bond_admin(None);
        set_pox_5_pause_admin(None);
        assert_eq!(pox_5_bond_admin(false), testnet_bond);
        assert_eq!(pox_5_pause_admin(false), testnet_pause);
    }
    ...
    fn mainnet_default_no_panic() {
        ...
        assert_eq!(pox_5_bond_admin(true), mainnet_bond);
        assert_eq!(pox_5_pause_admin(true), mainnet_pause);
    }
    ...
    fn mainnet_override_equal_to_default_ok() {
        let mainnet_bond = PrincipalData::parse(POX_5_BOND_ADMIN_MAINNET).unwrap();
        set_pox_5_bond_admin(Some(mainnet_bond.clone()));
        assert_eq!(pox_5_bond_admin(true), mainnet_bond);
        set_pox_5_bond_admin(None);
    }
    ...
    fn mainnet_bond_admin_override_panics() {
        let override_principal =
            PrincipalData::parse("ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM").unwrap();
        set_pox_5_bond_admin(Some(override_principal));
        let _ = pox_5_bond_admin(true);
    }
    ...
    fn mainnet_pause_admin_override_panics() {
        let override_principal =
            PrincipalData::parse("ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM").unwrap();
        set_pox_5_pause_admin(Some(override_principal));
        let _ = pox_5_pause_admin(true);
    }
}
