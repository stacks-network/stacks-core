...
use crate::chainstate::stacks::{
    Error, StacksPrivateKey, StacksPublicKey, TransactionAuth, TransactionAuthFlags,
...
impl TransactionAuth {
    ...
    pub fn verify_origin(&self, initial_sighash: &Txid) -> Result<Txid, net_error> {
        match *self {
            TransactionAuth::Standard(ref origin_condition) => Ok(
                origin_condition.verify(initial_sighash, &TransactionAuthFlags::AuthStandard)?
            ),
            TransactionAuth::Sponsored(ref origin_condition, _) => Ok(
                origin_condition.verify(initial_sighash, &TransactionAuthFlags::AuthStandard)?
            ),
        }
    }
...
mod test {
    ...
    use crate::chainstate::stacks::{
        ...
        StacksPublicKey as PubKey, TransactionAuthField, TransactionAuthFieldID,
