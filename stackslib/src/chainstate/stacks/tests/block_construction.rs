            ...
            |ref mut miner,
             ...
             ref parent_microblock_header_opt| {
                ...
                if tenure_id == 2 {
                    ...
                    // for tenure_id == 4:
                    // make a contract that, when called, will result in a RuntimeAnalysisError at
                    // runtime
                    ...
                    let runtime_analysis_error = format!(
                        "
                        ...
                        ;; triggers analysis_error at runtime because <trait> gets coerced
                        ...
                        ;; triggers a analysis_error at runtime because the code in
                        ;; `at-block` is buggy
                    ...
                    let runtime_analysis_error_trait_tx = make_user_contract_publish(
                        &privks_expensive[tenure_id],
                        1,
                        (2 * runtime_analysis_error_trait.len()) as u64,
                        "foo",
                        &runtime_analysis_error_trait
                    );

                    let runtime_analysis_error_impl_tx = make_user_contract_publish(
                        &privks_expensive[tenure_id],
                        2,
                        (2 * runtime_analysis_error_impl.len()) as u64,
                        "foo-impl",
                        &runtime_analysis_error_impl
                    );

                    let runtime_analysis_error_tx = make_user_contract_publish(
                        &privks_expensive[tenure_id],
                        3,
                        (2 * runtime_analysis_error.len()) as u64,
                        "trait-runtime-analysis-error",
                        &runtime_analysis_error
                    );

                    expected_txids.push(runtime_analysis_error_trait_tx.txid());
                    expected_txids.push(runtime_analysis_error_impl_tx.txid());
                    expected_txids.push(runtime_analysis_error_tx.txid());

                    for tx in &[&contract_call_spends_too_much_tx, &runtime_analysis_error_trait_tx, &runtime_analysis_error_impl_tx, &runtime_analysis_error_tx] {
                        mempool
                            .submit(
                                chainstate,
                                sortdb,
                ...
                if tenure_id == 4 {
                    ...
                    let runtime_analysis_error_problematic = make_user_contract_call(
                        &privks_expensive[tenure_id],
                        0,
                        2000,
                        &addrs_expensive[2],
                    ...
                    let err = StacksBlockBuilder::make_anchored_block_from_txs(
                        ...
                        chainstate,
                        &sortdb.index_handle_at_tip(),
                        vec![coinbase_tx.clone(), runtime_analysis_error_problematic.clone()]
                    );

                    if let Err(ChainstateError::ProblematicTransaction(ref txid)) = &err {
                        assert_eq!(txid, &runtime_analysis_error_problematic.txid());
                    }
                    else {
                        panic!("Did not get Error::ProblematicTransaction, but got {:?}", &err);
                    }

                    problematic_txids.push(runtime_analysis_error_problematic.txid());
                    mempool
                        .submit(
                            chainstate,
                            sortdb,
                            &parent_consensus_hash,
                            &parent_header_hash,
                            &runtime_analysis_error_problematic,
                            None,
                            &ExecutionCost::max_value(),
                            &StacksEpochId::Epoch2_05,
                        )
