...
mod utils {
    ...
    pub fn walk_to_insertion_point(
...
fn trie_cursor_try_attach_leaf(marf_opts: &MARFOpenOpts) {
    ...
    .iter()
    {
        ...
        for i in 0..32 {
            ...
            let (nodeptr, mut node, node_hash) = utils::walk_to_insertion_point(&mut f, &mut c);
...
fn trie_cursor_promote_leaf_to_node4(marf_opts: &MARFOpenOpts) {
    ...
    for i in 1..32 {
        ...
        let (nodeptr, node, node_hash) = utils::walk_to_insertion_point(&mut f, &mut c);
        // end of path -- cursor points to the insertion point
...
fn insert_1024_seq_low(marf_opts: &MARFOpenOpts) {
    utils::insert_n_test(marf_opts, true, 1024, |i| {
        [
            0,
            1,
            2,
...
fn insert_65536_random_deterministic(marf_opts: &MARFOpenOpts) {
    ...
    utils::insert_n_test(marf_opts, false, 65536, |i| {
        let mut path = [0; 32];
