    ...
    sim.execute_next_block_as_conn(|conn| {
        ...
        conn.as_transaction(|clarity_db| {
            ...
            let (mut ast, analysis) = clarity_db
                ...
                .unwrap();
            clarity_db
                .initialize_smart_contract(&contract_identifier, clarity_version, &mut ast, &analysis, contract, None, |_, _| None, &ResourceBudget::unlimited())
                .unwrap();
        });
        let mut tx = conn.start_transaction_processing();
...
fn publish_contract(
    ...
    bc.as_transaction(|tx| {
        let (mut ast, analysis) = tx.analyze_smart_contract(
            contract_id,
            version,
            contract,
            &ResourceBudget::unlimited(),
    ...
    sim.execute_next_block_as_conn(|conn| {
        ...
        conn.as_transaction(|clarity_db| {
            ...
            let (mut ast, analysis) = clarity_db.analyze_smart_contract(
                ...
                contract_clarity1, &ResourceBudget::unlimited(),
            ).unwrap();

            let res = clarity_db.analyze_smart_contract(
                &contract_identifier2,
                ClarityVersion::Clarity1,
                contract_clarity3, &ResourceBudget::unlimited(),
            );
            if let Err(ClarityError::StaticCheck(static_check_error)) = res {
    ...
    sim.execute_next_block_as_conn(|conn| {
        ...
        conn.as_transaction(|clarity_db| {
            ...
            let (mut ast, analysis) = clarity_db.analyze_smart_contract(
                &contract_identifier,
                ClarityVersion::Clarity3,
                contract, &ResourceBudget::unlimited(),
            ).unwrap();
