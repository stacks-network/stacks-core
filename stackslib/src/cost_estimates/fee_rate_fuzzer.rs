...
/// The FeeRateFuzzer wraps an underlying FeeEstimator. It passes `notify_block`
/// calls to the underlying estimator. On `get_rate_estimates` calls, it adds a
/// random fuzz to the result coming back from the underlying estimator. The
/// fuzz applied is as a random fraction of the base value.
///
/// Note: We currently use "uniform" random noise instead of "normal"
/// distributed noise to avoid importing a new crate just for this.
pub struct FeeRateFuzzer<UnderlyingEstimator: FeeEstimator> {
    ...
    /// Creator function for a new random generator. For prod, use `thread_rng`.
    /// For test, pass in a contrived generator.
    rng_creator: Box<dyn Fn() -> Box<dyn RngCore>>,
    /// The fuzzed rate will be `R * (1 + alpha)`, where `R` is the original
    /// rate, and `alpha` is a random number in `[-uniform_fuzz_fraction,
    /// uniform_fuzz_fraction]`. Note: Must be `0 <= uniform_fuzz_fraction <
    /// 1`.
    uniform_fuzz_fraction: f64,
}

impl<UnderlyingEstimator: FeeEstimator> FeeRateFuzzer<UnderlyingEstimator> {
    /// Constructor for production. It uses `thread_rng()` as the random number
    /// generator, to get truly pseudo-random numbers.
    pub fn new(
        underlying: UnderlyingEstimator,
        uniform_fuzz_fraction: f64,
    ...
    /// Constructor meant for test. The user can pass in a contrived random
    /// number generator factory function, so that the test is repeatable.
    ...
    /// Add a uniform fuzz to input. Each element is multiplied by the same
    /// random factor.
