...
;; todo(ludo): when transfering, we should unset revoked-at, imported-at fields
  ...
  (let (
    ...
    (let ( 
        ...
        (namespace-props (expects!
          (map-get namespaces ((namespace namespace)))
          (err err-namespace-not-found)))
        (can-sender-register-name (if (is-none? current-owned-name)
                                 'true
                                  (expects! 
      ...
      ;; The preorder must have been created after the launch of the namespace - todo(ludo): add a public fn for checking if a namespace is launched
      (asserts!
...
(define-public (name-update (namespace (buff 20))
  ...
  (let (
    ...
    (asserts!
      (eq? (expects! (is-name-lease-expired namespace name) (err err-panic)) 'false) ;; todo(ludo): refactor has-name-expired signature?
...
(begin
  (ft-mint! stx u100000 'SP2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKNRV9EJ7)
