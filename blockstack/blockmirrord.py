...
"""
    Blockstack mirror daemon, for Amazon S3
"""
...
def sync_blockchain( working_dir, first_block, last_block ):
   ...
   namespace_file = os.path.join( working_dir, config.BLOCKSTACKD_NAMESPACE_FILE)
   snapshots_file = os.path.join( working_dir, config.BLOCKSTACKD_SNAPSHOTS_FILE)
   lastblock_file = os.path.join( working_dir, config.BLOCKSTACKD_LASTBLOCK_FILE)
