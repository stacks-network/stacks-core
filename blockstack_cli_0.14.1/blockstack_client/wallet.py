...
def decrypt_wallet(data, password, config_path=CONFIG_PATH,
    ...
    for keyname in keynames:
        ...
        if not is_singlesig(ret[keyname_privkey]) and not multisig:
            msg = 'Invalid wallet data for "{}"'
            log.error(msg.format(keyname_privkey))
...
def unlock_wallet(password=None, config_dir=CONFIG_DIR, wallet_path=None):
    ...
    try:
        ...
        if onfile_data_pubkey is None:
            # make a data keypair (always the third child (index 2) of the
            # HDWallet)
            ...
            encrypted_wallet = make_wallet(
                password, hex_privkey=wallet['hex_privkey'],
            ...
            if 'error' in encrypted_wallet:
                log.error('Failed to make wallet: {}'.format(encrypted_wallet['error']))
                return encrypted_wallet

            write_wallet(encrypted_wallet, path='{}.tmp'.format(wallet_path))
            legacy_path = '{}.legacy'.format(wallet_path)
            if os.path.exists(wallet_path):
                if not os.path.exists(legacy_path):
                    ...
                    while os.path.exists(legacy_path):
                        legacy_path = '{}.legacy.{}'.format(wallet_path, i)
                        i += 1

                    shutil.move(wallet_path, legacy_path)

            shutil.move('{}.tmp'.format(wallet_path), wallet_path)
            msg = 'Migrated wallet {} (legacy wallet backed up to {})'
            log.debug(msg.format(wallet_path, legacy_path))
...
def get_addresses_from_file(config_dir=CONFIG_DIR, wallet_path=None):
    ...
    payment_address = data.get('payment_addresses')[0]
    owner_address = data.get('owner_addresses')[0]
    data_pubkey = data.get('data_pubkeys')[0]
...
def get_all_names_owned(wallet_path=WALLET_PATH):
    ...
    for entry in owner_addresses:
        ...
        names_owned.extend(name for name in additional_names)
...
def get_total_balance(config_path=CONFIG_PATH, wallet_path=WALLET_PATH):
    ...
    total_balance = sum(entry['balance'] for entry in payment_addresses)
