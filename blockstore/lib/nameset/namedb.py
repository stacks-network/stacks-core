...
class BlockstoreDB( virtualchain.StateEngine ):
    ...
    def __init__( self, db_filename, disposition ):
        """
        Construct the Blockstore State Engine
        ...
        super( BlockstoreDB, self ).__init__( magic_bytes,
                                              OPCODES,
                                              BlockstoreDB.make_opfields(),
                                              impl=blockstore_impl,
    ...
    def borrow_readwrite_instance( cls, db_path, block_number ):
        ...
        global blockstore_db, blockstore_db_lastblock, blockstore_db_lock

        blockstore_db_lock.acquire()

        try:
            assert blockstore_db is None, "Borrowing violation"
        except Exception, e:
            log.exception(e)
            log.error("FATAL: Borrowing violation")
            sys.exit(1)

        blockstore_db = BlockstoreDB( db_path )
        blockstore_db_lastblock = block_number
    ...
    def release_readwrite_instance( cls, db_inst, block_number ):
        ...
        try:
            assert blockstore_db is not None, "Borrowing return violation: db is None"
            assert blockstore_db == db_inst, "Borrowing return violation: different instances"
            assert blockstore_db_lastblock == block_number, "Borrowing return violation: different blocks"
        except Exception, e:
            log.exception(e)
    ...
    def sanitize_op( self, op_data ):
        ...
        op_data = super( BlockstoreDB, self ).sanitize_op( op_data )
    ...
    def find_collisions( self, checked_ops ):
        ...
        for op in checked_ops:
            if BlockstoreDB.nameop_is_collided( op ):
                BlockstoreDB.nameop_put_collision( collisions, op )
    ...
    def check_collision( self, history_id_key, history_id, block_id, checked_ops, affected_opcodes ):
        ...
        return BlockstoreDB.check_collision_state( self.collisions, history_id_key, history_id, block_id, checked_ops, affected_opcodes )
    ...
    def commit_state_preorder( self, nameop, current_block_number ):
        ...
        if BlockstoreDB.nameop_is_collided( nameop ):
