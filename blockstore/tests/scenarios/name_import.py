...
def scenario( wallets, **kw ):
    ...
    # import twice
    resp = testlib.blockstore_name_import( "bar.test", wallets[4].addr, "22" * 20, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )

    testlib.next_block( **kw )

    resp = testlib.blockstore_name_import( "bar.test", wallets[4].addr, "33" * 20, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
    ...
    resp = testlib.blockstore_name_import( "baz.test", wallets[5].addr, "44" * 20, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )

    resp = testlib.blockstore_name_import( "baz.test", wallets[5].addr, "55" * 20, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )

    resp = testlib.blockstore_name_import( "baz.test", wallets[5].addr, "66" * 20, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
    ...
    resp = testlib.blockstore_name_import( "foo.test", wallets[5].addr, "66" * 20, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )

    resp = testlib.blockstore_name_import( "bar.test", wallets[3].addr, "77" * 20, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )

    resp = testlib.blockstore_name_import( "baz.test", wallets[4].addr, "88" * 20, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
