...
def scenario( wallets, **kw ):
    ...
    resp = testlib.blockstore_name_transfer( "foo.test", wallets[3].addr, False, wallets[4].privkey )
    if debug or 'error' in resp:
        print json.dumps( resp, indent=4 )

    testlib.next_block( **kw )

    resp = testlib.blockstore_name_update( "foo.test", "33" * 20, wallets[3].privkey )
    if debug or 'error' in resp:
        print json.dumps( resp, indent=4 )
