...
pub fn strict_admits(me: &TypeSignature, x: &ClarityValue) -> Result<bool, RuntimeAnalysisError> {
    match me {
        ...
        TypeSignature::SequenceType(SequenceSubtype::StringType(StringSubtype::UTF8(
            ...
            if utf8_data
                .len()
                .map_or(false, |n| u32::from(n) > u32::from(my_max_len.clone()))
            {
                return Ok(false);
            }
            return Ok(true);
...
fn fuzz_sanitize(input: ClarityValue) {
    ...
    let deserialize_unsanitized =
        ClarityValue::deserialize_read(&mut serialized.as_slice(), Some(&computed_type), false);
    if !did_strict_admit {
        deserialize_unsanitized.unwrap_err();
    } else {
        let deser_value = match deserialize_unsanitized {
            Err(SerializationError::BadTypeError(RuntimeAnalysisError::TypeSignatureTooDeep)) => {
