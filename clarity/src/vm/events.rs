...
impl ContractCallEventData {
    pub fn json_serialize(&self) -> Result<serde_json::Value, SerializationError> {
        let function_args: Result<Vec<String>, SerializationError> =
            self.function_args.iter().map(hex_value).collect();
        Ok(json!({
            "contract_identifier": self.contract_identifier.to_string(),
            ...
            "raw_result": hex_value(&self.result)?,
        }))
    }
}
...
mod tests {
    ...
    fn var_set_event_json_serialization() {
        let event = VarSetEventData {
            contract_identifier: test_contract_id(),
            var_name: "n".into(),
            value: Value::UInt(7),
        };
        assert_eq!(
    ...
    fn vm_trace_event_uses_vm_event_index_not_event_index() {
        let event = VmTraceEvent::Storage(StorageEvent::VarSet(VarSetEventData {
            contract_identifier: test_contract_id(),
            var_name: "n".into(),
            value: Value::UInt(1),
        }));
        let json = event.json_serialize(3, &"deadbeef", true).unwrap();
