...
pub fn tuple_get(
    ...
    match value {
            ...
            match opt_data.data {
                Some(data) => {
                    if let Value::Tuple(tuple_data) = *data {
                        ...
                        Err(CheckErrorKind::ExpectedTuple(TypeSignature::type_of(&data)?).into())
                    }
                }
        ...
        _ => Err(CheckErrorKind::ExpectedTuple(TypeSignature::type_of(&value)?).into()),
    }
}
...
pub fn tuple_merge(base: Value, update: Value) -> Result<Value> {
    let initial_values = match base {
        ...
        _ => Err(CheckErrorKind::ExpectedTuple(TypeSignature::type_of(
            &base,
        )?)),
    }?;
