...
pub fn special_stx_balance(
    ...
    if let Value::Principal(ref principal) = owner {
        ...
        Err(CheckErrorKind::TypeValueError(TypeSignature::PrincipalType, owner).into())
    }
}
...
pub fn special_mint_asset_v200(
    ...
    if !expected_asset_type.admits(env.epoch(), &asset)? {
        return Err(CheckErrorKind::TypeValueError(expected_asset_type.clone(), asset).into());
    }
    ...
    if let Value::Principal(ref to_principal) = to {
        ...
        Err(CheckErrorKind::TypeValueError(TypeSignature::PrincipalType, to).into())
    }
}
...
pub fn special_burn_asset_v200(
    ...
    if let Value::Principal(ref sender_principal) = sender {
        ...
        Err(CheckErrorKind::TypeValueError(TypeSignature::PrincipalType, sender).into())
    }
}
