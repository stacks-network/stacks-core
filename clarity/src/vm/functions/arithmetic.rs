...
use crate::vm::errors::{
    check_argument_count, RuntimeAnalysisError, RuntimeError, VmExecutionError, VmInternalError,
};
...
macro_rules! type_force_binary_arithmetic {
    ($function: ident, $x: expr, $y: expr) => {{
        match ($x, $y) {
            ...
            (x, _) => Err(RuntimeAnalysisError::UnionTypeValueError(
...
macro_rules! type_force_unary_arithmetic {
    ($function: ident, $x: expr) => {{
        match $x {
            ...
            x => Err(RuntimeAnalysisError::UnionTypeValueError(
                vec![TypeSignature::IntType, TypeSignature::UIntType],
                Box::new(x),
            )
            .into()),
...
macro_rules! type_force_variadic_arithmetic {
    ($function: ident, $args: expr) => {{
        let first = $args
            .get(0)
            .ok_or(RuntimeAnalysisError::IncorrectArgumentCount(1, $args.len()))?;
        match first {
            Value::Int(_) => {
...
pub fn native_bitwise_left_shift(input: Value, pos: Value) -> Result<Value, VmExecutionError> {
    if let Value::UInt(u128_val) = pos {
        ...
        match input {
            ...
            _ => Err(RuntimeAnalysisError::UnionTypeError(
                vec![TypeSignature::IntType, TypeSignature::UIntType],
        ...
        Err(
            RuntimeAnalysisError::TypeValueError(Box::new(TypeSignature::UIntType), Box::new(pos))
                .into(),
        )
    }
}
...
pub fn native_to_uint(input: Value) -> Result<Value, VmExecutionError> {
    if let Value::Int(int_val) = input {
        ...
        Err(
            RuntimeAnalysisError::TypeValueError(Box::new(TypeSignature::IntType), Box::new(input))
                .into(),
        )
    }
}
