...
fn test_incomplete_impl_trait_1(#[case] version: ClarityVersion, #[case] epoch: StacksEpochId) {
    ...
    let err = db
        ...
        .unwrap_err();
    match *err.err {
        StaticAnalysisError::BadTraitImplementation(_, _) => {}
        _ => panic!("{err:?}"),
    }
}
...
fn test_dynamic_dispatch_by_defining_and_impl_trait(
    ...
    let err = db
        ...
        .unwrap_err();
    match *err.err {
        StaticAnalysisError::TraitReferenceUnknown(_) => {}
        _ => panic!("{err:?}"),
    }
}
...
fn test_cycle_in_traits_2_contracts(#[case] version: ClarityVersion, #[case] epoch: StacksEpochId) {
    ...
    let err = db
        ...
        .unwrap_err();
    match *err.err {
        StaticAnalysisError::NoSuchContract(_) => {}
        _ => panic!("{err:?}"),
    }
}
...
fn test_dynamic_dispatch_unknown_method(
    ...
    let err = db
        ...
        .unwrap_err();
    match *err.err {
        StaticAnalysisError::TraitMethodUnknown(_, _) => {}
        _ => panic!("{err:?}"),
    }
}
...
fn test_dynamic_dispatch_including_wrong_nested_trait(
    ...
    match *err.err {
        StaticAnalysisError::TypeError(expected, actual) if epoch < StacksEpochId::Epoch21 => {
        ...
        StaticAnalysisError::TypeError(expected, actual)
        ...
        StaticAnalysisError::TraitReferenceUnknown(name) => assert_eq!(name.as_str(), "trait-a"),
        _ => panic!("{err:?}"),
    }
}
...
fn test_dynamic_dispatch_mismatched_args(
    ...
    let err = db
        ...
        .unwrap_err();
    match *err.err {
        StaticAnalysisError::TypeError(_, _) => {}
        _ => panic!("{err:?}"),
    }
}
...
fn test_trait_contract_not_found(#[case] version: ClarityVersion, #[case] epoch: StacksEpochId) {
    ...
    match db.execute(|db| {
        ...
        Err(StaticAnalysisErrorReport { err, .. }) if version < ClarityVersion::Clarity2 => {
            match *err {
                StaticAnalysisError::NoSuchContract(contract) => {
