...
pub use crate::vm::analysis::errors::{
    ...
    CheckErrorKind, CheckResult, SyntaxBindingErrorType,
};
            ...
            FunctionType::Variadic(expected_type, _) => {
                ...
                if !admitted {
                    return (
                        cost,
                        Err(
                            CheckErrorKind::TypeError(expected_type.clone(), arg_type.clone())
                                .into(),
                        ),
                    );
                }
                ...
                if arg_index == 0 {
                    ...
                    let check_result = return_type.and_then(|return_type| {
                        if arg_type != return_type {
                            Err(
                                CheckErrorKind::TypeError(return_type.clone(), arg_type.clone())
                                    .into(),
                            )
                        } else {
                            Ok(None)
    ...
    pub fn check_args_2_1<T: CostTracker>(
        ...
        match self {
            ...
            FunctionType::UnionArgs(arg_types, return_type) => {
                ...
                Err(CheckErrorKind::UnionTypeError(arg_types.clone(), found_type.clone()).into())
            }
            ...
            | FunctionType::ArithmeticUnary => {
                ...
                for found_type in rest.iter() {
                    ...
                    if found_type != &return_type {
                        return Err(
                            CheckErrorKind::TypeError(return_type, found_type.clone()).into()
                        );
                    }
                }
                Ok(return_type)
            ...
            FunctionType::ArithmeticComparison => {
                ...
                if !pair_of_types_matches {
                    return Err(CheckErrorKind::TypeError(first.clone(), second.clone()).into());
                }
    ...
    match expected_sig {
        FunctionArgSignature::Single(expected_type) => {
            ...
            if !expected_type.admits_type(&StacksEpochId::Epoch21, actual_type)? {
                return Err(
                    CheckErrorKind::TypeError(expected_type.clone(), actual_type.clone()).into(),
                );
            }
        }
        ...
        match self.function_return_tracker {
            Some(ref mut tracker) => {
                let new_type = match tracker.take() {
                    ...
                    .map_err(|_| {
                        CheckErrorKind::ReturnTypesMustMatch(expected_type, return_type)
                    })?,
                    None => return_type,
                };
    ...
    fn type_check_function_type(
        ...
        if let Err(mut check_error) = check_result {
            if let CheckErrorKind::IncorrectArgumentCount(expected, _actual) = check_error.err {
                check_error.err = CheckErrorKind::IncorrectArgumentCount(expected, args.len());
            ...
            Ok(return_type) => {
                let return_type = {
                    if let Some(Some(ref expected)) = self.function_return_tracker {
                        ...
                        .map_err(|_| {
                            CheckErrorKind::ReturnTypesMustMatch(expected.clone(), return_type)
                        })?
                    } else {
                        return_type
                    }
    ...
    fn clarity1_type_check_expects(
        ...
        if !expected_type.admits_type(&StacksEpochId::Epoch21, &actual_type)? {
            let mut err: CheckError =
                CheckErrorKind::TypeError(expected_type.clone(), actual_type).into();
            ...
            match define_type {
                ...
                DefineFunctionsParsed::PublicFunction { signature, body } => {
                    ...
                    if f_type.returns.is_response_type() {
                        ...
                        return Err(CheckErrorKind::PublicFunctionMustReturnResponse(
                            f_type.returns,
                        )
                        .into());
                    }
