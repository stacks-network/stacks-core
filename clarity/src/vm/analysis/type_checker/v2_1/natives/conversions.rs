...
use crate::vm::analysis::StaticAnalysisErrorReport;
...
pub fn check_special_to_consensus_buff(
    ...
    .map_err(StaticAnalysisErrorReport::from)
}
...
pub fn check_special_from_consensus_buff(
    ...
    TypeSignature::new_option(result_type).map_err(StaticAnalysisErrorReport::from)
}
...
pub fn check_special_to_ascii(
    ...
    let result_type = match input_type {
        ...
        _ => {
            ...
            return Err(StaticAnalysisError::UnionTypeError(types, input_type.into()).into());
        }
    };
    Ok(
        TypeSignature::new_response(result_type, TypeSignature::UIntType).map_err(|_| {
            StaticAnalysisError::Expects("FATAL: Legal Clarity response type marked invalid".into())
        })?,
    )
}
