...
pub fn logn(n: u64, a: u64, b: u64) -> VmExecutionResult<u64> {
    if n < 1 {
        return Err(crate::vm::errors::VmExecutionError::Runtime(
...
pub fn nlogn(n: u64, a: u64, b: u64) -> VmExecutionResult<u64> {
    if n < 1 {
        return Err(crate::vm::errors::VmExecutionError::Runtime(
            RuntimeError::Arithmetic("log2 must be passed a positive integer".to_string()),
            Some(vec![]),
...
impl ClarityCostFunction {
    pub fn eval<C: CostValues>(&self, n: u64) -> VmExecutionResult<ExecutionCost> {
        match self {
