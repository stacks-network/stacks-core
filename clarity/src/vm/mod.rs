...
pub fn bench_create_var_in_context(
    ...
    global_context
        .add_memory(value.size().unwrap() as u64)
        .unwrap();

    let data_type = global_context
        .database
        .create_variable(&contract_context.contract_identifier, &name, value_type)
        .unwrap();
    global_context
        .database
        .set_variable(
...
pub fn bench_create_ft_in_context(
    ...
    contract_context.meta_ft.insert(name, data_type.unwrap());
}
...
pub fn bench_create_nft_in_context(
    ...
    contract_context.meta_nft.insert(name, data_type.unwrap());
}
