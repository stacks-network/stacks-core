...
//! Transaction post-condition checking.
//!
//! Post-conditions are a consensus-critical safety check applied *after* a
//! transaction's Clarity execution: they assert bounds on the assets that
//! actually moved (as aggregated in the [`AssetMap`]) and abort the
//! transaction if reality violates the sender's declared expectations.
//!
//! This logic lives in `clarity` rather than the node because its real inputs
//! are the codec post-condition types, an [`AssetMap`], the origin principal,
//! and the epoch — it has no database or chainstate dependency. Keeping the
//! single source of truth here lets the wasm SDK run the exact same check
//! mainnet uses, without pulling in `stackslib`.
...
use crate::vm::contexts::{AssetMap, AssetMapEntry};
