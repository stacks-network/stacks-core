...
proptest! {
    ...
    #[tag(t_prop)]
    #[test]
    fn prop_to_ascii_from_ascii_utf8_strings(
        (utf8_string, ascii_bytes) in utf8_string_ascii_only_snippet_strategy()
    ) {
        let snippet = format!("(to-ascii? {utf8_string})");
        let evaluation = evaluate_to_ascii(&snippet);

        let expected_inner = Value::string_ascii_from_bytes(ascii_bytes)
            .expect("generated bytes should form a valid ASCII string");
        let expected = Value::okay(expected_inner).expect("response wrapping should succeed");
