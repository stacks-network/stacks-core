...
fn map_no_contract_as_none<T>(res: Result<Option<T>>) -> Result<Option<T>> {
    res.or_else(|e| match e {
        VmExecutionError::IntegrityCheck(CheckErrorKind::NoSuchContract(_)) => Ok(None),
        x => Err(x),
    })
}
    ...
    pub fn set_variable(
            ...
            .admits(&self.get_clarity_epoch_version()?, &value)?
        {
            return Err(CheckErrorKind::TypeValueError(
                variable_descriptor.value_type.clone(),
                value,
            )
            .into());
        }
...
impl ClarityDatabase<'_> {
    ...
    fn inner_set_entry(
        ...
        {
            return Err(
                CheckErrorKind::TypeValueError(map_descriptor.key_type.clone(), key_value).into(),
            );
        }
        if !map_descriptor
            .value_type
        ...
        {
            return Err(
                CheckErrorKind::TypeValueError(map_descriptor.value_type.clone(), value).into(),
            );
        }
...
impl ClarityDatabase<'_> {
    ...
    pub fn get_nft_owner(
        ...
        if !key_type.admits(&self.get_clarity_epoch_version()?, asset)? {
            return Err(CheckErrorKind::TypeValueError(key_type.clone(), (*asset).clone()).into());
        }
