...
pub mod tests {
    ...
    use crate::vm::database::stores::null::ClarityNullStore;
    ...
    fn test_sanitization(#[case] version: ClarityVersion, #[case] epoch: StacksEpochId) {
        ...
        for (input_val, expected_out, good_type, bad_types) in test_cases.iter() {
            ...
            let result =
                RollbackWrapper::<ClarityNullStore>::deserialize_value(&serialized, good_type, &epoch).map(|x| x.value);
            if epoch < StacksEpochId::Epoch24 {
                let error = result.unwrap_err();
                assert!(matches!(error, crate::vm::database::v2::ClarityDbError::Serialization(SerializationError::DeserializeExpected(_))));
            } else {
            ...
            for bad_type in bad_types.iter() {
                ...
                let result = RollbackWrapper::<ClarityNullStore>::deserialize_value(&serialized, bad_type, &epoch);
                let error = result.unwrap_err();
                assert!(matches!(error, crate::vm::database::v2::ClarityDbError::Serialization(SerializationError::DeserializeExpected(_))));
            }
