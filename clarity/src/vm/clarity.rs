...
/// The top-level error type for the Clarity VM crate.
///
/// This enum represents a failure in any of the distinct operational phases
/// of the virtual machine.
#[derive(Debug)]
pub enum ClarityError {
    /// An error occurred during the parsing phase.
    ///
    /// This happens when the Clarity source code is syntactically incorrect and cannot be
    /// converted into an Abstract Syntax Tree (AST).
    Parse(ParseError),
    /// An error was found during the static analysis phase.
    ///
    /// Before execution, the AST is checked for correctness without running the code.
    /// This includes checks for type safety, function and variable definitions, and
    /// adherence to Clarity's language rules.
    StaticCheck(CheckError),
    /// An error occurred during the execution phase.
    ///
    /// This happens when the program's AST is being evaluated to produce a result.
    /// This variant wraps the comprehensive `VmExecutionError` type, which contains
    /// more specific details about the nature of the execution failure.
...
impl fmt::Display for ClarityError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            ClarityError::Execution(ref e) => fmt::Display::fmt(e, f),
...
impl std::error::Error for ClarityError {
    fn cause(&self) -> Option<&dyn std::error::Error> {
        match *self {
            ...
            ClarityError::Execution(ref e) => Some(e),
...
impl From<VmExecutionError> for ClarityError {
    fn from(e: VmExecutionError) -> Self {
        match &e {
            VmExecutionError::IntegrityCheck(CheckErrorKind::CostBalanceExceeded(a, b)) => {
                ClarityError::CostError(a.clone(), b.clone())
            }
            VmExecutionError::IntegrityCheck(CheckErrorKind::CostOverflow) => {
                ClarityError::CostError(ExecutionCost::max_value(), ExecutionCost::max_value())
            }
            VmExecutionError::IntegrityCheck(CheckErrorKind::ExecutionTimeExpired) => {
                ClarityError::CostError(ExecutionCost::max_value(), ExecutionCost::max_value())
            }
            _ => ClarityError::Execution(e),
        }
    }
}
