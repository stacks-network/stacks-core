...
impl StacksTransactionSigner {
    ...
    ) -> Result<StacksTransactionSigner, AuthError> {
        if !tx.auth.is_sponsored() {
            return Err(AuthError::IncompatibleSpendingConditionError);
        }
    ...
    pub fn sign_origin(&mut self, privk: &StacksPrivateKey) -> Result<(), AuthError> {
    ...
    pub fn append_origin(&mut self, pubk: &StacksPublicKey) -> Result<(), AuthError> {
        if self.check_overlap && self.origin_done {
    ...
    pub fn sign_sponsor(&mut self, privk: &StacksPrivateKey) -> Result<(), AuthError> {
    ...
    pub fn append_sponsor(&mut self, pubk: &StacksPublicKey) -> Result<(), AuthError> {
