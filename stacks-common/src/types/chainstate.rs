...
impl TrieHash {
    ...
    /// Create a `TrieHash` from a slice of bytes.
    #[inline] // sha2's new(), update() and finalize() fn's are inlined; ensure we don't break the inlining boundary.
    ...
    /// Create a `TrieHash` from an array of byte slices.
    #[inline] // sha2's new(), update() and finalize() fn's are inlined; ensure we don't break the inlining boundary.
